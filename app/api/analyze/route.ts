...
export async function POST(req: NextRequest) {
  ...
  if (!(await checkRateLimit(ip, '/api/analyze')).allowed) {
    return errorResponse('Rate limit exceeded. Try again in 60 seconds.', 429, { requestId }, {
      'Retry-After': '60',
      ...getCorsHeaders(req),
    });
  }
  ...
  if (!contentType.includes('application/json')) {
    return errorResponse('Invalid content type', 415, { requestId }, getCorsHeaders(req));
  }
  ...
  if (!groqKey && !geminiKey) {
    return errorResponse('No API keys configured', 500, { requestId }, getCorsHeaders(req));
  }
  ...
  if (rawSearchQuery) {
    if (rawSearchQuery.length > MAX_SEARCH_QUERY_CHARS) {
      return errorResponse(`searchQuery exceeds ${MAX_SEARCH_QUERY_CHARS} characters`, 413, { requestId }, getCorsHeaders(req));
    }
    ...
    if (!body?.contents || !Array.isArray(body.contents) || body.contents.length === 0) {
      return errorResponse('Missing contents array or searchQuery', 400, { requestId }, getCorsHeaders(req));
    }

    const extracted = extractPrompt(body);
    if (!extracted) {
      return errorResponse('No prompt text found', 400, { requestId }, getCorsHeaders(req));
    }
  ...
  if (prompt.length > MAX_PROMPT_CHARS + AXON_SYSTEM_PROMPT.length + 64) {
    return errorResponse(`Prompt exceeds ${MAX_PROMPT_CHARS} characters after assembly`, 413, { requestId }, getCorsHeaders(req));
  }

  const textOnlyRequest = rawSearchQuery ? true : isTextOnlyRequest(body);

  if (!rawSearchQuery && hasMultimodalContent(body) && !geminiKey) {
    return errorResponse('This request includes non-text content such as an image or file and requires GEMINI_API_KEY. Please configure it in your environment variables.', 503, { requestId }, getCorsHeaders(req));
  }
  ...
  return errorResponse('All AI providers are currently unavailable. Please try again in a moment.', 503, { requestId }, getCorsHeaders(req));
}
