...
export function getDependencyEdges(options?: {
  ...
  return WORKBENCH_DEPENDENCY_GRAPH.filter((edge) => {
    ...
    return options.direction === "upstream" ? edge.toToolId === options.toolId : edge.fromToolId === options.toolId;
  });
}

export function getUpstreamToolIds(toolId?: string | null, options?: { deep?: boolean; includeSupport?: boolean }) {
  if (!toolId) return [];
...
export function getDownstreamToolIds(toolId?: string | null, options?: { deep?: boolean; includeSupport?: boolean }) {
  if (!toolId) return [];
  const includeSupport = options?.includeSupport ?? true;
  const visited = new Set<string>();
...
export function getDependencyTrace(toolId?: string | null) {
  if (!toolId) return [];
  return getUpstreamToolIds(toolId, { deep: true })
    .reverse()
    .concat(toolId, getDownstreamToolIds(toolId, { deep: true }));
}
