...
function estimateProteinProperties(cds: string): {
  ...
  const hydrophobic =
    (aaCounts["F"] || 0) + (aaCounts["W"] || 0) + (aaCounts["I"] || 0) + (aaCounts["L"] || 0) + (aaCounts["V"] || 0);
...
export function predictGeneExpression(
  ...
  return {
    ...
    host: construct.host,
    constructLength:
      construct.promoter.length + construct.rbs.length + construct.cds.length + construct.terminator.length,
