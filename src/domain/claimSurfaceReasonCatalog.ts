...
export const CLAIM_SURFACE_REASON_CATALOG: Record<ClaimSurfaceBlockCode, ClaimSurfaceReason> = {
  MISSING_POLICY: {
    ...
    suggestedAction: "Add a policy entry for the tool and surface before allowing formal consumption.",
  },
  TIER_NOT_ALLOWED_FOR_SURFACE: {
    blockCode: "TIER_NOT_ALLOWED_FOR_SURFACE",
    title: "Validity tier is not allowed on this surface",
    explanation: "The output validity tier is below the threshold declared for this claim surface.",
    suggestedAction: "Use the output only on a less formal surface or replace it with partial or real evidence.",
  },
  PROVENANCE_REQUIRED: {
    ...
    suggestedAction: "Re-run or publish the output with provenance before using it on this surface.",
  },
  HUMAN_GATE_REQUIRED: {
    ...
    suggestedAction: "Route the output through the future human-gate workflow before continuing.",
  },
  DRAFT_OUTPUT_NOT_EXPORTABLE: {
    ...
    suggestedAction: "Commit or finalize the output with provenance before exporting it.",
  },
  DEMO_OUTPUT_PROTOCOL_BLOCKED: {
    blockCode: "DEMO_OUTPUT_PROTOCOL_BLOCKED",
    title: "Demo output cannot generate protocol",
    explanation: "Demo-level outputs may be useful for learning, but cannot become protocol-like artifacts.",
    suggestedAction: "Use the output as exploratory context only or replace it with partial or real evidence.",
  },
...
export function getClaimSurfaceReason(blockCode: ClaimSurfaceBlockCode): ClaimSurfaceReason {
