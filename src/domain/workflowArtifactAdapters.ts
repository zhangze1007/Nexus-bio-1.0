...
function deriveTargetProduct(artifact: WorkflowArtifact) {
  ...
  return (
    artifact.intake.targetMolecule ??
    terminalNode?.label ??
...
function summarizeRoute(
  route: WorkflowArtifact["candidateRoutes"][number],
...
export function deriveAnalyzeCompatibilityProjection(artifact: WorkflowArtifact): WorkbenchAnalyzeArtifact {
  ...
  const bottleneckNodes = nodes
    ...
    .slice(0, 4);
  const enzymeNodes = nodes.filter((node) => node.role === "enzyme").slice(0, 4);
  const thermodynamicConcerns =
    artifact.thermodynamics?.concerns ??
    edges
      .filter(
        (edge) =>
  ...
  const recommendedNextTools = Array.from(
    new Set(["pathd", "fbasim", "cethx", ...(enzymeNodes.length > 0 ? ["catdes"] : [])]),
  );
  ...
  return {
    ...
    summary:
      artifact.intake.projectIntent ?? `Compiled ${nodes.length} nodes and ${edges.length} edges for ${targetProduct}.`,
    targetProduct,
    nodes,
    edges,
