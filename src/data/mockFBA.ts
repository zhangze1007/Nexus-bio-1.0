 ...
 */
export function runFBA(glucoseUptake: number, oxygenUptake: number, knockouts: string[] = []): FBAOutput {
...
export function computeFBAResult(glucoseUptake: number, oxygenUptake: number): FBAResult {
  ...
  const reactions = BASE_REACTIONS.map((r) => ({
    ...r,
    flux: r.subsystem === "TCA" ? (r.flux ?? 0) * scale * oScale : (r.flux ?? 0) * scale,
  }));
  return {
...
function _computeYeastFluxes(
  ...
  if (koSet.has("HXT") || koSet.has("TPI")) {
    ...
    if (koSet.has("ACS")) {
      raw["IDH"] = 0;
      raw["BIOMASS_y"] = (raw["BIOMASS_y"] ?? 0) * 0.4;
    }
  }
  return { raw, aerobic };
}
 ...
 */
export function runYeastFBA(glucoseUptake: number, oxygenUptake: number, knockouts: string[] = []): FBAOutput {
