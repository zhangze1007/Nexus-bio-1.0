 ...
 */
export async function searchPubChemCompound(name: string): Promise<FallbackResult<PubChemCompound>> {
  ...
  return fetchWithFallback(
    async () => {
      const res = await fetch(`/api/pubchem?name=${encodeURIComponent(name)}`, { signal: AbortSignal.timeout(10000) });
      ...
      try {
        const propRes = await fetch(`/api/pubchem?cid=${cid}&properties=true`, { signal: AbortSignal.timeout(10000) });
        if (propRes.ok) {
