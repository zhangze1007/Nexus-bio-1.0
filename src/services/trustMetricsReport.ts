...
function mismatchReason(
  ...
  if (expectedBlockCode !== actualBlockCode) {
    reasons.push(`blockCode ${actualBlockCode ?? "<empty>"} did not match expected ${expectedBlockCode ?? "<empty>"}`);
  }
  return reasons.join("; ");
}
...
function buildMismatches(evaluatedCases: EvaluatedTrustBenchmarkCase[]): TrustMetricMismatch[] {
  ...
  for (const item of evaluatedCases) {
    ...
    mismatches.push({
      ...
      reason: mismatchReason(expectedStatus, item.actualStatus, expectedBlockCodeValue, item.actualBlockCode),
    });
  }
...
function requiredKnownBadCoverage(cases: TrustBenchmarkMetricCase[]) {
  const represented = new Set(cases.filter((testCase) => testCase.knownBad).flatMap((testCase) => testCase.riskTags));
...
export function buildTrustMetricsReport(input: BuildTrustMetricsReportInput): TrustFalsificationMetrics {
  ...
  const provenanceRequiredBlocks = evaluatedCases.filter((item) => item.actualBlockCode === "PROVENANCE_REQUIRED");
  ...
  return {
    ...
    unsafeExportPreventionRate: roundedRate(preventedUnsafeFormalCases.length, unsafeFormalCases.length),
    ...
    knownBadCoverageRate: roundedRate(knownBadCoverage.representedTags.length, knownBadCoverage.requiredTags.length),
...
export function toTrustMetricsHistoryEntry(report: TrustFalsificationMetrics): TrustMetricsHistoryEntry {
  return {
...
export function parseExpectedLabelsCsv(csv: string): TrustBenchmarkExpectedLabel[] {
  ...
  return lines.filter(Boolean).map((line, index) => {
    ...
    const [caseId, expectedStatus, expectedBlockCode, category, toolId, surface, knownBad] = parts;
...
function parseHumanGateStatus(value: unknown, context: string): TrustBenchmarkHumanGateStatus | undefined {
  ...
  if (value === "not-required" || value === "pending" || value === "approved" || value === "rejected") {
    return value;
  }
...
function parseBenchmarkCase(value: unknown, context: string): TrustBenchmarkMetricCase {
  ...
  if (uncertaintyState !== "bounded" && uncertaintyState !== "unresolved" && uncertaintyState !== "not-applicable") {
