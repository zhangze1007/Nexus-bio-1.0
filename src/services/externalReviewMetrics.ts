...
export function computeExternalReviewMetrics(
  submission: ExternalReviewSubmission,
): ExternalReviewMetrics {
  ...
  for (const response of submission.responses) {
    if (response.reviewerStatus === 'unsure') {
      ...
      response.reviewerStatus
      === comparisonTarget(response.runtimeStatus, response.expectedStatus)
    ) {
      agreementCount += 1;
    } else {
      disagreementCount += 1;
    ...
    if (
      response.wordingMisleading === true
      || response.suggestedAction === 'rewrite-wording'
    ) {
      wordingIssueCount += 1;
    }
  }
  ...
  const bypassSuccessCount = submission.adversarialAttempts.filter(
    (attempt) => attempt.bypassSucceeded,
  ).length;
