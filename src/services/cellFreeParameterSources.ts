...
export const PARAMETER_SOURCES: Record<string, ParameterSource> = {
  ...
  k_tx_sigma70: {
    value: 0.8,
    unit: 'nM/min',
    source: 'Heuristic — sigma70 ~3-5x weaker than T7; T7 kcat ~230 nt/s vs sigma70 ~50 nt/s (BRENDA EC 2.7.7.6 vs 2.7.7.60)',
    doi: '',
    status: 'heuristic',
  },
  k_tx_ptac: {
  ...
  t7_rnap_kcat: {
    value: 4.2,
    unit: 'nt/s',
    source: 'BRENDA: EC 2.7.7.6, T7 RNA polymerase — reported range 200-300 nt/s; 4.2 nt/s = 252 nt/min scaled to nM product',
  ...
  initial_atp: {
    value: 1.5,
    unit: 'mM',
    source: 'Estimated — typical S30 extract energy charge; Calhoun & Swartz 2005 report ~1-2 mM ATP in optimized extracts',
  ...
  initial_pep: {
    value: 33,
    unit: 'mM',
    source: 'Estimated — PEP is the primary energy substrate; Jewett & Swartz 2004 use 33 mM PEP for energy regeneration',
  ...
  initial_amino_acids: {
    value: 15.0,
    unit: 'mM',
    source: 'Estimated — Shimizu et al. 2001 (PURE system) use ~1.5 mM each of 20 amino acids ≈ 30 mM total; S30 extracts typically 10-20 mM',
  ...
  K_CONSUME_TL: {
    ...
    source: 'Heuristic — ~4 ATP per amino acid (2 for tRNA charging + 2 for elongation), ~300 aa protein; Russell & Cook 1995',
    doi: '',
    status: 'heuristic',
  },
  K_GTP_CONSUME: {
  ...
  mock_vmax: {
    value: 450,
    unit: 'RFU/min',
    source: 'Synthetic ground truth for demo — not from measurement; typical sfGFP expression in S30 reaches ~500-2000 RFU/min',
    doi: '',
    status: 'estimated',
  },
  mock_kd: {
    value: 8.5,
    unit: 'nM',
    source: 'Synthetic ground truth for demo — not from measurement; typical plasmid Kd for cell-free expression ~5-20 nM',
    doi: '',
    status: 'estimated',
  },
};
