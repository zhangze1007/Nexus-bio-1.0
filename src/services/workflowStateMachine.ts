 ...
 */
import { type Actor, assign, createActor, setup } from "xstate";
import {
...
export function meetsContract(toolId: GoldenPathToolId, status: WorkflowToolStatus): boolean {
...
export const workflowMachine = setup({
  ...
  actions: {
    setTarget: assign({
      targetProduct: ({ event }) => (event.type === "SET_TARGET" ? event.targetProduct : null),
    }),
    clearAll: assign(() => initialContext),
    recordPathd: assign({
      toolStatus: ({ context, event }) =>
        event.type === "PATHD_DONE" ? { ...context.toolStatus, pathd: event.status } : context.toolStatus,
    }),
    recordFbasim: assign({
      toolStatus: ({ context, event }) =>
        event.type === "FBASIM_DONE" ? { ...context.toolStatus, fbasim: event.status } : context.toolStatus,
    }),
    recordCatdes: assign({
      toolStatus: ({ context, event }) =>
        event.type === "CATDES_DONE" ? { ...context.toolStatus, catdes: event.status } : context.toolStatus,
    }),
    recordDyncon: assign({
      toolStatus: ({ context, event }) =>
        event.type === "DYNCON_DONE" ? { ...context.toolStatus, dyncon: event.status } : context.toolStatus,
    }),
    recordCellfree: assign({
      toolStatus: ({ context, event }) =>
        event.type === "CELLFREE_DONE" ? { ...context.toolStatus, cellfree: event.status } : context.toolStatus,
    }),
    recordDbtl: assign({
      toolStatus: ({ context, event }) =>
        event.type === "DBTL_COMMITTED" ? { ...context.toolStatus, dbtlflow: event.status } : context.toolStatus,
    }),
    applySidecar: assign({
      toolStatus: ({ context, event }) =>
        event.type === "SIDECAR_UPDATE" ? { ...context.toolStatus, [event.toolId]: event.status } : context.toolStatus,
...
export function describeWorkflowState(value: WorkflowStateValue): string {
  switch (value) {
    case "idle":
      return "No target product set.";
    case "targetSet":
      return "Target set; awaiting PATHD pathway design.";
    case "pathdReady":
      return "PATHD complete; awaiting FBASim flux analysis.";
    case "fbasimReady":
      return "FBASim complete; awaiting CatDes catalyst optimization.";
    case "catdesReady":
      return "CatDes complete; awaiting DynCon controller tuning.";
    case "dynconReady":
      return "DynCon complete; awaiting CellFree validation.";
    case "cellfreeReady":
      return "CellFree complete; awaiting DBTLflow Learn commit.";
    case "dbtlCommitted":
      return "DBTL Learn committed; ready to LOOP_BACK or finalize.";
    default:
      return value satisfies never;
  }
}

export const ALL_GOLDEN_PATH_DONE_EVENTS = GOLDEN_PATH_TOOL_IDS.map((id) => GOLDEN_PATH_DONE_EVENT[id]);
