...
import type { RetentionPolicy, RetentionEnforcementResult, RetentionStatusEntry, ArchiveResult } from "./types";
 ...
 */
export async function enforceRetentionPolicies(orgId: string): Promise<RetentionEnforcementResult> {
 ...
 */
export async function getRetentionStatus(orgId: string): Promise<RetentionStatusEntry[]> {
  if (!orgId || typeof orgId !== "string" || orgId.trim().length === 0) {
    throw new Error("orgId is required and must be a non-empty string");
  }
 ...
 */
export async function archiveExpiredData(entityType: string, cutoffDate: Date): Promise<ArchiveResult> {
      ...
      try {
        await sqlRun(
          `INSERT INTO archived_records (id, original_table, original_id, data, archived_at, archived_by)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [randomUUID(), entityType, String(record.id ?? ""), JSON.stringify(record), now, "retention-automation"],
        );
        await sqlRun(`UPDATE ${entityType} SET archived = 1 WHERE id = ?`, [record.id]);
