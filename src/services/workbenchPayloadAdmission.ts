...
export function evaluateWorkbenchPayloadAdmission(
  ...
  return {
    decision,
    mode,
    shouldWritePayload: mode === "observe" ? true : decision.status === "ok" || decision.status === "demoOnly",
  };
}
...
export function inferAdmissionInputFromPayload(args: {
  ...
  const provenanceValidity = isValidityTier(provenanceRecord.validityTier) ? provenanceRecord.validityTier : undefined;
  const humanGateStatus = isHumanGateStatus(record.humanGateStatus)
    ...
    : typeof record.humanGateApproved === "boolean"
      ? record.humanGateApproved
        ? "approved"
        : "pending"
      : undefined;
  const isDraft = typeof record.isDraft === "boolean" ? record.isDraft : record.feedbackSource === "draft";
  ...
  return {
    ...
    requiresHumanGate: typeof record.requiresHumanGate === "boolean" ? record.requiresHumanGate : undefined,
    humanGateStatus,
  };
}
