...
function validateCondition(
  ...
  if (
    ...
    || !(POLICY_DSL_CONDITION_FIELDS as readonly string[]).includes(value.field)
  ) {
    pushIssue(
      issues,
      `${fieldPath}.field`,
...
function validateRule(
  ...
  if (
    ...
    && (
      ...
      || !(OVERRIDE_PATHS as readonly string[]).includes(value.overridePath)
    )
  ) {
    pushIssue(
      issues,
...
function validateDefaultDecision(
  value: unknown,
  issues: PolicyDslValidationIssue[],
): void {
  if (!isRecord(value)) {
    pushIssue(
      ...
      'defaultDecision',
      'MISSING_REQUIRED_FIELD',
      'defaultDecision must be an object.',
    );
    return;
  }
  ...
  if (value.effect !== 'block') {
    pushIssue(
      ...
      'INVALID_EFFECT',
      'defaultDecision.effect must be block.',
    );
  }
...
export function validatePolicyDslDocument(value: unknown): PolicyDslValidationResult {
  ...
  if (value.schemaVersion === undefined) {
    ...
    pushIssue(
      ...
      'schemaVersion',
      'INVALID_SCHEMA_VERSION',
      `schemaVersion must be ${POLICY_DSL_SCHEMA_VERSION}.`,
    );
  }
