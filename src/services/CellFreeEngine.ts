 ...
 *
 * 4. In-vitro-to-In-vivo (IvIv) Translator — heuristic estimate
 *
 * References:
...
// ═══════════════════════════════════════════════════════════════
//  3. estimateIvIvHeuristic
// ═══════════════════════════════════════════════════════════════

/**
 * Estimate in-vivo gene expression from in-vitro cell-free data using a
 * heuristic model with deterministic seeded weights.  This is a heuristic
 * estimate, not a trained model — the weights are generated by SeededRNG(12345)
 * and have not been fitted to experimental data.
 *
 * Applies biological correction factors (folding, codon bias, promoter
 * context, RBS sequestration) to adjust the raw estimate.
 *
 * @param input - in-vitro measurements and construct descriptors
 * @returns estimated in-vivo expression, confidence, corrections and reasoning
 */
export function estimateIvIvHeuristic(input: IvIvInput): IvIvPrediction {
  ...
  const reasoning = [
    ...
    `base heuristic estimate ${Math.round(basePrediction)} molecules/cell. `,
