 ...
 */
export function generateCacheKey(prompt: string, model: string, params: Record<string, unknown> = {}): string {
...
export async function getCachedResponse(key: string): Promise<CachedResponse | null> {
  ...
  const row = await sqlGet("SELECT * FROM ai_cache WHERE cache_key = ? AND expires_at > ?", [key, now]);
...
export async function cacheSize(): Promise<number> {
  await ensureSchema();
  const now = new Date().toISOString();
  const rows = await sqlAll("SELECT COUNT(*) as cnt FROM ai_cache WHERE expires_at > ?", [now]);
