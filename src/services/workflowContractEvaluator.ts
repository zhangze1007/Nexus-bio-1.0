...
export function evaluateToolContract(
  ...
  const validity =
    payload && typeof payload === "object" ? ((payload as { validity?: ValidityFloor }).validity ?? null) : null;
  ...
  const validityOk = validity !== null && meetsValidityFloor(validity, contract.validityBaseline.floor);
  const confidenceOk =
    contract.confidencePolicy.minToAdvance === null ||
    (confidence !== null && confidence >= contract.confidencePolicy.minToAdvance);
  const uncertaintyOk = !contract.uncertaintyPolicy.unboundedIsGate || uncertainty !== null;
  const evidence = options.evidence ?? [];
  const haveKinds = new Set(
    evidence
      .map((item) => item.sourceKind)
      .filter(
        (kind): kind is EvidenceSourceKind =>
  ...
  const evidenceOk = evidence.length >= contract.evidenceRequired.minItems && missingEvidenceKinds.length === 0;
