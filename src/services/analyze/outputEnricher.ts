...
function hasDesignFields(value: unknown): boolean {
  ...
  return (
    typeof v.active_site_remodeling === "string" &&
...
export function enrichAxonOutput(raw: string): EnrichResult {
  ...
  const bottlenecks = nodes
    ...
    .map((node) => {
      const efficiency =
        readNumber(node.efficiency_percent) ??
  ...
  if (primary) {
    ...
    question =
      "No critical bottlenecks detected. The pathway appears thermodynamically favorable. Should we explore cofactor optimization or investigate potential downstream processing constraints?";
