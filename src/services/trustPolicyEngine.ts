...
function tierBlockCode(validityTier: ValidityTier, surface: ClaimSurface): ClaimSurfaceBlockCode {
...
export function evaluateClaimSurfacePolicy(input: EvaluateClaimSurfacePolicyInput): GateDecision {
  ...
  if (!input.validityTier) {
    return decision({
      ...
      reason: `A validity tier is required before ${input.toolId} output can be evaluated for ${input.surface}.`,
      overridePath: "not-allowed",
    });
  }
  ...
  if (input.isDraft === true && policy.denyIfDraft === true) {
    return decision({
      ...
      reason: `${input.toolId} output is still draft and ${input.surface} requires a finalized artifact.`,
  ...
  if (!policy.allowedTiers.includes(input.validityTier)) {
    ...
    return decision({
      ...
      surface: input.surface,
      blockCode,
      reason: `${input.validityTier} output from ${input.toolId} is not allowed on ${input.surface}.`,
      overridePath: "not-allowed",
    });
  }
  ...
  if (policy.requiresProvenance && !hasItems(input.provenanceIds)) {
    return decision({
      ...
      reason: `${input.surface} consumption for ${input.toolId} requires provenance before the claim can be used.`,
      overridePath: "human-review",
    });
  }
  ...
  if (humanGateRequired && humanGateStatus !== "approved") {
    if (humanGateStatus === "rejected") {
      return decision({
        ...
        reason: `Human review rejected ${input.toolId} output for ${input.surface}.`,
        overridePath: "not-allowed",
      });
    }
    ...
    return decision({
      ...
      surface: input.surface,
      blockCode: "HUMAN_GATE_REQUIRED",
      reason: `${input.surface} consumption for ${input.toolId} requires approved human review.`,
      overridePath: "human-review",
    });
  }
  ...
  if (input.validityTier === "demo") {
    return decision({
      status: "demoOnly",
      surface: input.surface,
      reason: `${input.toolId} output is allowed on ${input.surface} only as demo or exploratory context.`,
    });
  }
  ...
  return decision({
    status: "ok",
    surface: input.surface,
    reason: `${input.toolId} output satisfies the ${input.surface} claim-surface policy.`,
  });
}
