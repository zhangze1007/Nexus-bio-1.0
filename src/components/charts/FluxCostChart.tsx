...
import { CHART_CONTAINER, FONT, fmt2, SCI_PALETTE, SECTION_LABEL } from "./chartTheme";
...
function ProgressBar({ value, max, color, limitAt }: { value: number; max: number; color: string; limitAt?: number }) {
  ...
  return (
    ...
    >
      <div
        style={{ width: `${pct}%`, height: "100%", borderRadius: 7, background: color, transition: "width 0.4s ease" }}
      />
      {limitPct != null && (
        <div
          style={{
            ...
            bottom: -3,
            width: 1,
            borderLeft: `2px dashed ${colors.text.secondary}`,
          }}
        >
          <span
            style={{
  ...
  return (
    <div style={{ ...CHART_CONTAINER, background: colors.bg.primary, padding: Number(spacing.base) }}>
      ...
      <p style={{ fontFamily: FONT.SANS, fontSize: 9, color: colors.text.tertiary, margin: `0 0 ${spacing.sm}` }}>
        Bars share an axis for visual comparison but carry heterogeneous units — hover each bar for its native quantity.
        Engine emits single-point estimates, so no whiskers are drawn.
      </p>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 20, marginBottom: 14 }}>
          <div>
            <p style={{ fontFamily: FONT.MONO, fontSize: 28, color: colors.text.primary, margin: 0, lineHeight: 1 }}>
              {fmt2(drainPct)}%
            </p>
            <p style={{ fontFamily: FONT.SANS, fontSize: 9, color: colors.text.secondary, margin: "2px 0 0" }}>
          ...
          <div
            style={{
              padding: "6px 12px",
              borderRadius: 10,
              background: `${viabilityColor}14`,
              border: `1px solid ${viabilityColor}40`,
            }}
          >
            <p
              style={{
        ...
        {/* Drain progress bar */}
        <p style={{ fontFamily: FONT.SANS, fontSize: 10, color: colors.text.primary, margin: "0 0 4px" }}>
          Metabolic drain (%)
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ fontFamily: FONT.MONO, fontSize: 10, color: colors.text.primary, flexShrink: 0 }}>
            {fmt2(drainPct)}%
          </span>
        </div>
