...
function GlassTooltip({ active, payload, label }: ChartTooltipProps) {
  ...
  return (
    <div style={TOOLTIP_STYLE}>
      <p style={{ margin: 0, fontSize: 10, color: colors.text.tertiary, fontFamily: FONT.SANS }}>Iteration {label}</p>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 0, flexShrink: 0 }}>
          ...
          <div
            style={{
              width: 0,
              height: 0,
              borderLeft: `4px solid ${colors.border.strong}`,
        ...
        >
          <span
            style={{
              fontFamily: FONT.MONO,
              fontSize: 10,
              color: result.isBalanced ? SCI_PALETTE.green : SCI_PALETTE.vermilion,
            }}
          >
      ...
      {convergenceData.length > 1 && (
        <>
          ...
          <p style={{ fontFamily: FONT.SANS, fontSize: 9, color: colors.text.tertiary, margin: `0 0 ${spacing.sm}` }}>
            Single deterministic optimization trace — no confidence band is drawn because a confidence band would imply
            repeated runs that do not exist.
          </p>
