...
function GlassTooltip({ active, payload, label }: ChartTooltipProps) {
  ...
  return (
    <div style={TOOLTIP_STYLE}>
      <p style={{ margin: 0, fontSize: 10, color: colors.text.tertiary, fontFamily: FONT.SANS }}>{label}</p>
...
function MetricCell({ label, value, unit, color }: { label: string; value: string; unit?: string; color?: string }) {
  return (
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <p
        style={{ fontFamily: FONT.MONO, fontSize: 18, color: color || colors.text.primary, margin: 0, lineHeight: 1.1 }}
      >
        {value}
...
export default function BindingRadarChart({ result }: BindingRadarChartProps) {
  ...
  const fitQuality = result.overallScore >= 0.8 ? "Excellent" : result.overallScore >= 0.6 ? "Moderate" : "Weak";
    ...
    <div style={{ ...CHART_CONTAINER, background: colors.bg.primary, padding: Number(spacing.base) }}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              tickLine={false}
            />
            <YAxis dataKey="name" type="category" tick={rechartsTick} axisLine={false} tickLine={false} width={90} />
      ...
      >
        ...
        <MetricCell
          ...
          value={`${fmt2(result.bindingEnergy)} ± ${fmt2(Math.abs(result.bindingEnergy) * 0.1)}`}
          unit="kcal/mol"
      ...
      <div style={{ width: "100%", height: 200 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={decompositionData} margin={{ top: 8, right: 16, left: 10, bottom: 4 }} barSize={40}>
