...
import type { EnzymeStructure, MutagenesisResult } from "../../services/CatalystDesignerEngine";
...
export default function MutagenesisChart({ result, enzyme }: MutagenesisChartProps) {
  ...
  return (
    <div style={{ ...CHART_CONTAINER, background: colors.bg.primary, padding: Number(spacing.base) }}>
      ...
      >
        Mutagenesis effects require external tools (FoldX, Rosetta ddg_monomer, protein design suites) — no quantitative
      ...
      <svg
        role="img"
        aria-label="Mutagenesis sequence bar"
        viewBox={`0 0 500 56`}
      ...
      >
        ...
        {Array.from({ length: Math.ceil(seqLen / 50) + 1 }).map((_, i) => {
          ...
          return (
            <g key={pos}>
              ...
              <text x={tx} y={52} textAnchor="middle" fontFamily={FONT.MONO} fontSize="10" fill={colors.text.disabled}>
                {pos}
              </text>
            </g>
          );
        ...
        <text x="10" y="6" fontFamily={FONT.SANS} fontSize="10" fill={colors.text.secondary}>
          Catalytic
        </text>
        <rect x="60" y="0" width="6" height="6" fill={ACCENT.mint} rx={1} />
        <text x="70" y="6" fontFamily={FONT.SANS} fontSize="10" fill={colors.text.secondary}>
          Mutagenesis
        </text>
      </svg>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={siteData} margin={{ top: 8, right: 16, left: 10, bottom: 4 }} barSize={28}>
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 3"
              label={{ value: "WT", fill: colors.text.disabled, fontSize: 8, fontFamily: FONT.MONO, position: "right" }}
            />
          ...
          <tbody>
            {result.sites.map((s) => (
              <tr key={s.position} style={{ borderBottom: `1px solid ${colors.border.subtle}` }}>
                <td style={{ fontFamily: FONT.MONO, fontSize: 10, color: ACCENT.mint, padding: "4px 5px" }}>
                  {s.position}
                </td>
                <td
                  style={{
                ...
                <td style={{ fontFamily: FONT.MONO, fontSize: 9, color: colors.text.primary, padding: "4px 5px" }}>
                ...
                <td
                  style={{
                    ...
                    fontSize: 9,
                    padding: "4px 5px",
                    color: effectColor(s.predictedEffect),
                  }}
                >
                  {s.predictedEffect}
                </td>
                ...
                >
                  {s.predictedDeltaKcat != null ? `${fmt2(s.predictedDeltaKcat)}x` : "N/A"}
                </td>
                <td
                  style={{
                    fontFamily: FONT.MONO,
                ...
                >
                  {s.predictedDeltaKm != null ? `${fmt2(s.predictedDeltaKm)}x` : "N/A"}
                </td>
                <td
                  style={{
                    fontFamily: FONT.MONO,
