...
import { loadGoalContext, clearGoalContext, findStepIndex, type GoalContext } from '../lib/goal-context';
...
export default function WorkflowBanner() {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: THEME.INK, fontWeight: 600, flexShrink: 0, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {ctx.goal}
      </span>
          ...
          return (
            <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
              <span style={{
                ...
                fontSize: 'var(--nb-fs-xxs)', fontWeight: 600,
                background: isDone ? 'rgba(147,203,82,0.15)' : isCurrent ? 'rgba(195,215,232,0.2)' : 'rgba(255,255,255,0.05)',
      ...
      <button onClick={handleDismiss} style={{
        ...
        color: THEME.LABEL, cursor: 'pointer', fontSize: '14px', padding: '2px 6px',
      }} aria-label="退出工作流">✕</button>
    </div>
  );
}
