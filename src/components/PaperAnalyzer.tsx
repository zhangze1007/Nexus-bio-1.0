...
Rules: 4-8 nodes, IDs lowercase with underscores, labels 1-3 words max.`;
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // More robust JSON extraction — find first { to last }
      const jsonMatch = raw.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error('NO_JSON');
      const parsed = JSON.parse(jsonMatch[0]);
      if (!parsed.nodes || !parsed.edges) throw new Error('NO_JSON');
      ...
      if (msg === 'NO_JSON' || msg.includes('JSON') || msg.includes('format')) {
        setError('AI response was incomplete. Please wait a moment and try again.');
      } else if (msg.includes('429')) {
        setError('Too many requests. Please wait 1–2 minutes before trying again.');
      } else {
