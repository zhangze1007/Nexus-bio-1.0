...
// ─── 智能形态分类引擎：根据分子属性分配形状 ─────────────────────────────
type ShapeType = 'box' | 'tetra' | 'sphere';

function getGeomForType(type?: string): ShapeType {
  if (!type) return 'sphere';
  const t = type.toLowerCase();
  // 酶类/蛋白质 -> 四方格
  if (t.includes('enzyme') || t.includes('protein') || t.includes('catalyst') || t.includes('synthase') || t.includes('reductase') || t.includes('oxidase')) return 'box';
  // 中间体/前体 -> 三角形 (四面体)
  if (t.includes('intermediate') || t.includes('precursor')) return 'tetra';
  // 默认代谢物/化合物 -> 圆形
  return 'sphere';
}

function GeoComp({ g, s }: { g: ShapeType; s: number }) {
  switch(g) {
    case 'box':   return <boxGeometry args={[s * 1.2, s * 1.2, s * 1.2]} />; // 适当放大四方格以匹配球体体积
    case 'tetra': return <tetrahedronGeometry args={[s * 1.3, 0]} />; // 三角形
    default:      return <sphereGeometry args={[s, 32, 32]} />; // 极其平滑的圆形
  }
}
...
// ─── 分子节点 ──────────────────────────────────────────────────────────
const MolNode = React.memo(function MolNode({ node, hov, sel, onClick, onHov, cc }: {
  ...
  // 获取专属形态
  const shapeType = useMemo(() => getGeomForType(node.nodeType), [node.nodeType]);
  
  // 星轨配置
  const spinSpeed = useMemo(() => hashFloat(node.id, 6, 0.04, 0.10), [node.id]);
  ...
  useFrame((state, dt) => {
    ...
    if (grp.current) {
      ...
      grp.current.rotation.z = t * spinSpeed * 0.5;
    }
  ...
  return (
    ...
    >
      <mesh ref={bodyRef}>
        <GeoComp g={shapeType} s={0.32 + cc * 0.04} />
        {/* 移除噪点贴图，回归极其平滑的玻璃质感 */}
        <meshPhysicalMaterial
          color={finalColor} emissive={finalColor} emissiveIntensity={0.05}
          roughness={0.2} metalness={0.1} transmission={isHighAlert ? 0 : 0.4} thickness={0.5}
        />
      </mesh>
...
const PathEdge = React.memo(function PathEdge({ edge, s, e, active, color }: { edge:PathwayEdge; s:Vec3; e:Vec3; active:boolean; color:string }) {
  ...
  return (
    <group>
      <Line 
        points={[sv, mid, ev]} color={active ? color : '#556677'} 
        lineWidth={active ? thickness * 1.5 : thickness} transparent opacity={active ? 0.8 : 0.25} 
      />
      {/* 修复凹凸感：将低像素 (5x5) 替换为极致平滑的圆球 (32x32) 并使用无杂光的纯粹发光材质 */}
...
// ─── Main Component (完整找回 WebGL Fallback) ─────────────────────────
...
export default function ThreeScene({ nodes, onNodeClick, edges, selectedNodeId }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ pointerEvents: 'none', position:'absolute', top:0, left:0, right:0, zIndex:10, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 18px', background:'linear-gradient(to bottom, rgba(16,18,22,0.95), transparent)', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
      ...
      <div style={{ pointerEvents: 'none', position:'absolute', bottom:'16px', left:'16px', zIndex:10 }}>
      ...
      <div style={{ pointerEvents: 'none', position:'absolute', bottom:'16px', right:'16px', zIndex:10, background:'rgba(10,12,16,0.6)', padding:'8px 12px', borderRadius:'10px', border:'1px solid rgba(255,255,255,0.08)', backdropFilter:'blur(10px)' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
        <SceneErrorBoundary onError={(e) => setStatus('error')}>
          <Canvas
            ...
            dpr={[1, 1.5]} performance={{ min: 0.5 }} style={{ pointerEvents: 'auto' }}
          >
            <ResizeHandler />
