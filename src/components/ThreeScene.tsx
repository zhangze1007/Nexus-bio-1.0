...
function normalizeOpticalInsets(fullscreen: boolean, insets?: Partial<OpticalInsetBox>): OpticalInsetBox {
...
function getOpticalTargetOffset(args: {
  ...
  return new Vector3(-desiredNdcX * frustumHalfWidth * 0.4, -desiredNdcY * frustumHalfHeight * 0.4, 0);
}
...
function computeCameraFrame(args: {
  ...
  return {
    ...
    opticalTargetOffset,
    opticalTarget,
    cameraPosition: new Vector3(0, size.y * CAMERA_ELEVATION_RATIO, distance),
  };
}
...
function getRendererLabel(mode: RendererMode): string | null {
  switch (mode) {
    case "loading":
      return "INITIALIZING";
    case "webgl2":
      return null; // WebGL2 is the target renderer — no label needed
    case "webgl":
      return "LEGACY WEBGL";
    case "error":
...
function getColor(node: PathwayNode): string {
  const isHighRisk =
    node.color_mapping === "Red" ||
    node.nodeType === "impurity" ||
    (node.risk_score !== undefined && node.risk_score > HIGH_RISK_THRESHOLD);
...
function AmbientParticles() {
  ...
  const initData = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      positions.push([r * Math.cos(theta), Math.sin(i * 0.37) * 6, r * Math.sin(theta)]);
    }
    return positions;
  }, []);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      dummy.position.set(
        ...
        bz + Math.cos(t * 0.1 + phases[i] * 0.7) * 0.35,
      );
  ...
  return (
    ...
    <instancedMesh ref={meshRef} args={[undefined, undefined, PARTICLE_COUNT]} frustumCulled={true}>
      ...
      <meshLambertMaterial transparent opacity={0.55} depthWrite={false} vertexColors />
    </instancedMesh>
  );
}
...
const MolNode = React.memo(function MolNode({
  node,
  hov,
  sel,
  cc,
  ...
  const tgt = sel ? 1.28 : hov ? 1.1 : 1.0;
        ...
        <Html distanceFactor={10} center style={{ pointerEvents: "none", zIndex: 100 }}>
          ...
          >
            ...
            >
              ...
              <span style={{ color: "rgba(255,255,255,0.3)", fontSize: "10px", fontFeatureSettings: "'tnum' 1" }}>
                {Math.round(conf * 100)}%
              </span>
            </div>
            ...
            <p style={{ color: "rgba(180,200,215,0.42)", fontSize: "11px", lineHeight: 1.6, margin: "0 0 7px" }}>
    ...
    <group>
      ...
      <mesh ref={dot} visible={false}>
        ...
        <meshLambertMaterial
          color={color}
          emissive={color}
          emissiveIntensity={isSpontaneous ? 0.8 : 0.6}
          transparent
    ...
    () =>
      edges
        .map((e) => {
          ...
          return {
            sv: new Vector3(...(s.position as [number, number, number])),
            ev: new Vector3(...(t.position as [number, number, number])),
          };
        })
...
function Scene({
  ...
  const onEnd = useCallback(() => {
    timer.current = setTimeout(() => setInteract(false), 3500);
  }, []);
  ...
  return (
    <>
      ...
      <pointLight
        position={[0, routeGeometry.size.y * 0.5 + 6, 0]}
        intensity={0.18 * glowMultiplier}
        color="#FFFFFF"
        distance={28}
        decay={2}
      />
      ...
      <group position={routeGeometry.routeGroupOffset.toArray() as Vec3}>
        <FluxParticles
          edges={edges}
          nodes={nodes}
          flowSpeed={viewMode === "flow" ? flowSpeed * 1.25 : flowSpeed}
      ...
      <ScrollSyncCamera
        ...
        baseTarget={opticalTarget}
        routeCenter={routeGeometry.boundsCenter}
        opticalOffset={opticalTargetOffset}
      />
    </>
  );
}
...
interface Props {
  ...
  fullscreen?: boolean;
  stressIndex?: number;
  opticalInsets?: Partial<OpticalInsetBox>;
...
export default function ThreeScene({
  ...
  const resolvedTraceLayout = useMemo(
    () => ({
      ...
      left: tracePlacement === "top-left" ? (traceLayout?.left ?? resolvedOpticalInsets.left) : traceLayout?.left,
      right: tracePlacement === "top-right" ? (traceLayout?.right ?? resolvedOpticalInsets.right) : traceLayout?.right,
  ...
  const riskNodes = useMemo(
    () => safeNodes.filter((node) => (node.risk_score ?? 0) >= HIGH_RISK_THRESHOLD).length,
    [safeNodes],
  );
  const spontaneousEdges = useMemo(
    () => safeEdges.filter((edge) => (edge.predicted_delta_G_kJ_mol ?? 0) < 0).length,
    [safeEdges],
  );
  ...
  const connectedEdges = useMemo(
    () =>
      selectedNodeId ? safeEdges.filter((edge) => edge.start === selectedNodeId || edge.end === selectedNodeId) : [],
  ...
  const modeTrace = useMemo(() => {
    if (viewMode === "risk") {
      return {
        ...
        summary: selectedNode
          ? selectedNode.audit_trail ||
            selectedNode.toxicity_impact ||
            selectedNode.dsp_bottleneck ||
            "Risk view emphasizes audit trail, toxicity, and downstream bottlenecks."
    ...
    if (viewMode === "flow") {
      ...
      return {
        ...
        summary:
          ...
          edgeEvidence?.audit_trail ||
          "Flux view highlights mapped reaction flow and spontaneity-linked edges.",
    ...
    return {
      ...
      metric: selectedNode
        ? `confidence ${getConfidenceValue(selectedNode).toFixed(2)}`
        : `${safeNodes.length} mapped entities`,
    };
  ...
  return (
    <div
      style={{
        ...
        borderRadius: "0",
        overflow: "hidden",
        border: fullscreen ? "none" : "0.5px solid rgba(255,255,255,0.07)",
        position: "relative",
        boxShadow: "none",
      }}
    >
      ...
      <div
        style={{
          ...
          padding: "11px 16px",
          background: "linear-gradient(to bottom, rgba(16,16,16,0.92), transparent)",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span style={{ color: "rgba(255,255,255,0.10)", fontSize: "10px", fontFamily: THEME.SANS }}>
            drag · scroll · click
          </span>
        </div>
      </div>
      ...
      <div
        style={{
          pointerEvents: "none",
          position: "absolute",
          bottom: `${resolvedOpticalInsets.bottom}px`,
          left: `${resolvedOpticalInsets.left}px`,
          zIndex: 10,
        }}
      >
        <p
      ...
      <div
        style={{
          ...
          right:
            tracePlacement === "top-right" && resolvedTraceLayout.right !== undefined
              ? `${resolvedTraceLayout.right}px`
              : "auto",
          left:
            tracePlacement === "top-left" && resolvedTraceLayout.left !== undefined
              ? `${resolvedTraceLayout.left}px`
              : "auto",
          zIndex: 10,
          ...
          background:
            "linear-gradient(180deg, rgba(255,255,255,0.10) 0%, rgba(242,247,252,0.06) 22%, rgba(8,10,14,0.48) 100%)",
          padding: "8px 10px",
      ...
      >
        ...
        {[
          ...
          <div key={x.l} style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "3px" }}>
            <span style={{ color: x.c, fontSize: "10px", lineHeight: 1, width: "10px", textAlign: "center" }}>
              {x.s}
            </span>
      ...
      >
        ...
        >
          ...
          <Scene
            ...
            opticalInsets={resolvedOpticalInsets}
            debugFrameName={debugFrameName}
          />
        </Canvas>
      </SceneErrorBoundary>
      {status !== "ready" && (
        ...
        >
          ...
          >
            <strong
              style={{ display: "block", color: "rgba(255,255,255,0.88)", fontSize: "11px", marginBottom: "3px" }}
            >
