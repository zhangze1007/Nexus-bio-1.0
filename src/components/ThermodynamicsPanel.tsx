...
import { calculateEnzymeKinetics, estimateActivationEnergy, eyringRateConstant } from "../utils/eyringKinetics";
...
function MiniChart({ x, y, color, label }: { x: number[]; y: number[]; color: string; label: string }) {
  const W = 280,
    H = 80,
    PAD = { t: 6, r: 8, b: 20, l: 38 };
  const iW = W - PAD.l - PAD.r;
  const iH = H - PAD.t - PAD.b;
  const xMin = x[0],
    xMax = x[x.length - 1] || 1;
  const yMin = 0,
    yMax = Math.max(...y) * 1.15 || 1;
  ...
  return (
    <div>
      ...
      <svg width={W} height={H}>
        ...
        {pts && (
          <polygon
            points={`${px(x[0])},${PAD.t + iH} ${pts} ${px(x[x.length - 1])},${PAD.t + iH}`}
            fill={color}
            fillOpacity={0.08}
          />
        )}
...
function PathwayWaterfallChart({
  steps,
}: {
  steps: { label: string; dG0: number; dGTransformed: number; cumulative: number }[];
}) {
  const W = 280,
    H = 140,
    PAD = { t: 10, r: 8, b: 30, l: 42 };
  const iW = W - PAD.l - PAD.r;
  const iH = H - PAD.t - PAD.b;
  ...
  return (
    <svg width={W} height={H} style={{ display: "block" }}>
      ...
      <text
        ...
        fontSize={7}
        fontFamily={THEME.SANS}
        transform={`rotate(-90, 4, ${PAD.t + iH / 2})`}
      >
        kJ/mol
      </text>
          ...
          <g key={i}>
            ...
            {i > 0 && (
              <line
                x1={barX(i - 1) + barW}
                x2={x}
                y1={valToY(steps[i - 1].cumulative)}
                y2={valToY(steps[i - 1].cumulative)}
                stroke="rgba(200,216,232,0.2)"
                strokeWidth={0.5}
                strokeDasharray="2,2"
              />
            )}
            {/* Step bar */}
            <rect x={x} y={barY} width={barW} height={barHeight} fill={color} fillOpacity={0.6} rx={2} />
            {/* Value label */}
            <text
              x={x + barW / 2}
              y={step.dGTransformed >= 0 ? yTop - 4 : yTop + barHeight + 10}
              textAnchor="middle"
              fill={color}
              fontSize={7}
              fontFamily={THEME.SANS}
            ...
            {/* Cumulative dot */}
            <circle cx={x + barW / 2} cy={valToY(step.cumulative)} r={2.5} fill="#C8D8E8" stroke="none" />
            {/* Step label */}
            <text
              x={x + barW / 2}
              y={H - 8}
...
export default function ThermodynamicsPanel({ nodeLabel, nodeId, ecNumber: initialEcNumber }: ThermoPanelProps) {
  ...
  const run = async () => {
    ...
    const stepDG0s = pathwayStepsInput
      .split(",")
    ...
    const pathwayTotal = stepDG0s.length > 0 ? calcPathwayDeltaG(pathwayStepObjs, pH, ionicStrength, T) : 0;
          ...
          contents: [
            {
              parts: [
                {
                  text: `You are a biochemist. Interpret this thermodynamic analysis for the metabolite ${nodeLabel}:
...
Do not return JSON, code fences, or developer-style logs.`,
      ...
      setAi({
        text:
          text.trim() ||
          buildThermodynamicFallbackInterpretation({
            dG,
            dG0,
            keq: Keq,
            nodeLabel,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
      <div
        style={{
          padding: "10px 12px",
          borderRadius: "16px",
          background: "rgba(200,224,208,0.05)",
      ...
      <div
        style={{
          ...
          border: "1px solid rgba(221,208,232,0.1)",
        }}
      >
      ...
      >
        ...
        <InputF
          label="Step ΔG° values"
          value={pathwayStepsInput}
          unit="kJ/mol (comma-sep)"
          onChange={setPathwayStepsInput}
          type="text"
      ...
      {ecNumber && (
        <div
          style={{
            padding: "8px 12px",
            borderRadius: "12px",
            background: brendaData ? "rgba(200,224,208,0.05)" : "rgba(200,200,200,0.05)",
            border: `1px solid ${brendaData ? "rgba(200,224,208,0.1)" : "rgba(200,200,200,0.1)"}`,
          }}
        >
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <span
              style={{
                color: brendaData ? "rgba(200,224,208,0.6)" : "rgba(200,200,200,0.4)",
                fontSize: "10px",
                fontFamily: THEME.SANS,
                fontFeatureSettings: "'tnum' 1",
                textTransform: "uppercase",
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        <ActionButton variant="primary" size="md" icon={<Play size={13} />} onClick={run} style={{ flex: 1 }}>
          Calculate ΔG
        </ActionButton>
        {result && (
          <ActionButton
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
            {[
              ...
              {
                l: "Spontaneous",
                v: result.spontaneous ? "Yes (exergonic)" : "No (endergonic)",
                col: result.spontaneous ? "#C8E0D0" : "#E8C8D4",
              },
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
              {[
                {
                  l: "ΔG'° (transformed)",
                  v: `${result.dGTransformed.toFixed(2)} kJ/mol`,
                  col: result.dGTransformed < 0 ? "#DDD0E8" : "#E8C8D4",
                },
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                {[
                  ...
                  {
                    l: "Effective kcat",
                    v: `${result.kinetics.kcat_eff.toFixed(2)} 1/s`,
                    col: "rgba(255,255,255,0.6)",
                  },
                  { l: "Effective Km", v: `${result.kinetics.km_eff.toFixed(4)} mM`, col: "rgba(255,255,255,0.6)" },
                  ...(result.kinetics.inhibition > 0
                    ? [{ l: "Inhibition", v: `${(result.kinetics.inhibition * 100).toFixed(1)}%`, col: "#E8C8D4" }]
                    : []),
                  ...(result.eyringRate
                    ? [{ l: "Eyring k", v: `${result.eyringRate.toExponential(2)} 1/s`, col: "rgba(255,255,255,0.5)" }]
                    : []),
                ].map((m) => (
                  <div
                    key={m.l}
