        ...
        <tbody>
          {shown.map((row, i) => (
            <tr key={i} style={{ background: i % 2 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
              {headers.map(h => (
                <td key={h} style={{ padding: '4px 8px', color: THEME.VALUE, borderBottom: `1px solid ${THEME.BORDER}`, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '200px' }}>
                  {row[h]}
                </td>
              ))}
            </tr>
