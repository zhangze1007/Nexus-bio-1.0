...
export default function EmptyState({ type = "empty", title, message, action }: EmptyStateProps) {
  ...
  const accent = type === "error" ? THEME.CORAL : type === "loading" ? THEME.SKY : THEME.MINT;
    ...
    >
      ...
      >
        {type === "loading" ? (
          ...
          <span style={{ fontFamily: MONO, fontSize: "14px", color: accent }}>{type === "error" ? "!" : "·"}</span>
        )}
      </div>
