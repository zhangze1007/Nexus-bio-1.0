...
export default function DatabaseStatusDashboard() {
  ...
  useEffect(() => {
    const checkDatabase = async (name: string, url: string): Promise<"live" | "mock"> => {
      try {
    ...
    Promise.all([
      ...
      setDatabases((prev) => prev.map((db, i) => ({ ...db, status: results[i] })));
    });
  }, []);
        ...
        >
          <span
            style={{
              ...
              height: "6px",
              borderRadius: "50%",
              background: db.status === "live" ? "#4ade80" : db.status === "mock" ? "#fbbf24" : "#666",
            }}
          />
          {db.name}
        </span>
