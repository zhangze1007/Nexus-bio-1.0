...
export default function AxonPlanPanel({ plan, compact }: AxonPlanPanelProps) {
  if (!plan) {
    return (
      ...
      >
        No active plan. Ask Axon to plan a pathway design or flux analysis and the steps will appear here with
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
        ...
        <span style={{ fontFamily: THEME.MONO, fontSize: "10px", color: THEME.VALUE }}>{summarisePlan(plan)}</span>
        <span style={{ fontFamily: THEME.MONO, fontSize: "10px", color: THEME.LABEL }}>origin={plan.origin}</span>
      </div>
      <div
        style={{
          fontFamily: THEME.SANS,
      ...
      {plan.warnings.length > 0 && (
        ...
        >
          {plan.warnings.map((w, i) => (
            <div key={i} style={{ fontFamily: THEME.SANS, fontSize: "10px", color: "#E8C49A", lineHeight: 1.4 }}>
              {w}
            </div>
          ))}
        </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "grid", gap: "3px", minWidth: 0 }}>
                ...
                >
                  <div>
                    <strong style={{ color: THEME.VALUE }}>Objective:</strong> {step.objective}
                  </div>
                  <div>
                    <strong style={{ color: THEME.VALUE }}>Input:</strong> {step.inputSummary}
                  </div>
                  <div>
                    <strong style={{ color: THEME.VALUE }}>Expected:</strong> {step.expectedOutput}
                  </div>
