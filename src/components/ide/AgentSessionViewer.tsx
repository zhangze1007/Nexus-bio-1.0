...
import { sessionStatusLabel, sessionStepStatusLabel } from "../../services/axonSessionView";
    ...
    case "context":
      return (
        <PreviewFrame title="Workbench context">
          ...
          <KeyValRow label="Evidence" value={`${preview.evidenceSelected}/${preview.evidenceTotal} selected`} />
      ...
      return (
        <PreviewFrame title="Metadata">
          {preview.entries.length === 0 ? (
            <div style={{ fontFamily: THEME.SANS, fontSize: "10px", color: THEME.LABEL }}>No metadata captured.</div>
          ) : (
            preview.entries.map((e, i) => <KeyValRow key={i} label={e.label} value={e.value} />)
          )}
        </PreviewFrame>
      );
    case "result":
    ...
    case "off-domain":
      return (
        <PreviewFrame title="Off-domain advisory">
          ...
          <KeyValRow label="Signals" value={preview.signals.length > 0 ? preview.signals.join(", ") : "—"} />
          <div style={{ fontFamily: THEME.SANS, fontSize: "10px", color: "#CFC4E3", lineHeight: 1.45 }}>
            The biosynthesis planner was not invoked. Rephrase with pathway, enzyme, or flux-analysis intent to route
            this through a Nexus-Bio tool.
          </div>
        </PreviewFrame>
      );
    case "unavailable":
...
function StepCard({ step, isCurrent }: { step: AxonSessionStep; isCurrent: boolean }) {
...
function OutcomeCard({ session }: { session: AxonSession }) {
  ...
  const messages: Record<AxonSessionStatus, string> = {
    ...
    interrupted:
      "Previous session ended mid-run (page reload or navigation). Running tasks were marked interrupted, not failed.",
    "off-domain":
      "Request routed outside Nexus-Bio scope. Planner was not invoked and no biosynthesis prompt was called.",
...
export default function AgentSessionViewer({ session, hideHeader, maxSteps = 40 }: AgentSessionViewerProps) {
  ...
  if (session.status === "idle" && steps.length === 0) {
    return (
      ...
      >
        ...
        Ask Axon a question to start a real session. The viewer will show the classifier decision, the plan, each tool
        step, evidence adapter status, writebacks, and every lifecycle phase — nothing is fabricated.
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        {steps.length === 0 ? (
          ...
          steps.map((step) => <StepCard key={step.id} step={step} isCurrent={session.currentStepId === step.id} />)
        )}
      </div>
    </div>
  );
