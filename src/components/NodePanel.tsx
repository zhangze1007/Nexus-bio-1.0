...
import { X, Download, FileText, Hash, Link2, AlertCircle } from 'lucide-react';
...
export default function NodePanel({ node, onClose, allNodes, allEdges }: NodePanelProps) {
  ...
  const handleDownload = () => {
    ...
    a.setAttribute('download', `${node.id}_node.json`);
    document.body.appendChild(a);
    a.click();
    a.remove();
  };

  // Derive connections from edge data
  const connections = allEdges?.filter(e => e.start === node?.id || e.end === node?.id) || [];
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              {node.confidenceScore !== undefined && (
                <ConfidenceBar score={node.confidenceScore} />
              )}
