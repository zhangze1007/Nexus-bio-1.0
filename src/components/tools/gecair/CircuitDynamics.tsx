...
import type { GateType, LogicCascadeState, RepressilatorState, ToggleSwitchState } from "../../../data/mockGECAIR";
import { runLogicCascade, runRepressilator, runToggleSwitch } from "../../../data/mockGECAIR";
...
export function ODEMiniDynamics({
  circuitType,
  togglePerturbation,
}: {
  ...
  if (circuitType === "repressilator") {
    ...
    return (
      ...
      >
        ...
        >
          3-node ring oscillator: LacI represses TetR, TetR represses cI, cI represses LacI. Produces sustained
          limit-cycle oscillations (Elowitz &amp; Leibler, 2000).
        </div>
  ...
  if (circuitType === "logic_cascade") {
    ...
    return (
      ...
      >
        ...
        >
          3-node linear cascade: A constitutively driven, B repressed by A, C repressed by B. Signal attenuation through
          the cascade enables noise filtering (Hooshangi et al., 2005).
        </div>
        <svg width={w} height={h} style={{ display: "block", width: "100%" }}>
          <path d={toPath("pA")} fill="none" stroke="#C8D8E8" strokeWidth={1.5} />
          <path d={toPath("pB")} fill="none" stroke="#C8E0D0" strokeWidth={1.5} />
        ...
        <div style={{ fontFamily: THEME.MONO, fontSize: "11px", color: THEME.mint, marginTop: "6px" }}>
          Cascade gain: {cascadeGain.toFixed(2)} (pA={finalPA.toFixed(1)}, pB={finalPB.toFixed(1)}, pC=
          {finalPC.toFixed(1)})
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      >
        Mutual repression bistable switch: A represses B, B represses A. Settles to one stable state depending on
        initial perturbation (Gardner et al., 2000).
      </div>
      <svg width={w} height={h} style={{ display: "block", width: "100%" }}>
        <path d={toPath("pA")} fill="none" stroke="#C8D8E8" strokeWidth={1.5} />
    ...
    <div style={{ padding: "16px" }}>
      ...
      {(() => {
        ...
        if (circuitType === "repressilator") {
          ...
          const maxP = Math.max(...trajectory.flatMap((s) => [s.pA, s.pB, s.pC]));
          const toPath = (key: keyof RepressilatorState) => {
            const pts = trajectory.map(
              (s, i) => `${mL + (i / trajectory.length) * plotW},${mT + plotH - (s[key] / maxP) * plotH}`,
            );
            return `M${pts.join(" L")}`;
          };
          return (
            <ScientificFigureFrame
              ...
              title="Repressilator — RK4 Simulation"
              caption="3-node ring oscillator: LacI→TetR→cI→LacI. Sustained limit-cycle oscillations."
            >
        ...
        if (circuitType === "logic_cascade") {
          ...
          return (
            <ScientificFigureFrame
              eyebrow="ODE Dynamics"
              title="Logic Cascade — RK4 Simulation"
              caption="3-node linear cascade with signal attenuation for noise filtering."
            >
              <svg width={w} height={h} style={{ display: "block", width: "100%" }} viewBox={`0 0 ${w} ${h}`}>
                <path d={toPath("pA")} fill="none" stroke="#C8D8E8" strokeWidth={2} />
                <path d={toPath("pB")} fill="none" stroke="#C8E0D0" strokeWidth={2} />
        ...
        return (
          <ScientificFigureFrame
            eyebrow="ODE Dynamics"
            title="Toggle Switch — RK4 Simulation"
            caption={`Bistable switch. Perturbation: State ${togglePerturbation}. Settled to state ${settledState}.`}
          >
      ...
      {stochasticMode &&
        ...
        (() => {
          ...
          const runLines = proteinIds.flatMap((id) => {
            ...
            >
              <svg width={w} height={h} style={{ display: "block", width: "100%" }} viewBox={`0 0 ${w} ${h}`}>
                ...
                {runLines.map((rl) => (
                  <path
                    ...
                    d={rl.path}
                    fill="none"
                    stroke={speciesColors[rl.id] || "#888"}
                    strokeWidth={0.6}
                    opacity={0.25}
                  />
                ))}
              ...
              <div
                style={{
                  marginTop: "12px",
                  display: "grid",
                  gridTemplateColumns: `repeat(${proteinIds.length}, 1fr)`,
                  gap: "8px",
                }}
              >
                {summary.map((s) => (
                  ...
                  >
                    ...
                    <div style={{ fontFamily: THEME.MONO, fontSize: "10px", color: THEME.LABEL, lineHeight: 1.6 }}>
                      <div>
                        Mean: {s.mean.toFixed(1)} +/- {s.std.toFixed(1)}
                      </div>
