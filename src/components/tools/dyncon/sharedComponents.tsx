...
import { DEFAULT_PARAMS, hillFeedback } from "../../../data/mockDynCon";
import { toolTokens } from "../../../hooks/useToolTheme";
import { THEME } from "../../../theme";
import type { HillParams, ODEState } from "../../../types";
...
export function TimeSeriesSVG({
  ...
  const lanes = [
    ...
    { key: "dissolvedO2", label: "DO₂", color: `${THEME.SKY}E6`, max: 1, unit: "sat." },
    {
      key: "fpp",
      label: "FPP",
      color: `${THEME.CORAL}F0`,
      max: Math.max(0.001, ...trajectory.map((point) => point.fpp ?? 0)),
      unit: "μM",
    },
    {
      key: "adsExpression",
      label: "ADS",
      color: `${THEME.LILAC}EB`,
      max: Math.max(0.001, ...trajectory.map((point) => point.adsExpression ?? 0)),
      unit: "a.u.",
    },
  ] as const;
  ...
  return (
    <SVGChartContainer W={W} H={H + 36} ariaLabel="Closed-loop trajectory" variant="paper" svgRef={svgRef}>
      ...
      {lanes.map((lane, index) => {
        ...
        const setpointY = lane.key === "dissolvedO2" ? y + laneH - normalize(setpoint, lane.max) * laneH : null;
        const toxicityY =
          lane.key === "fpp" ? y + laneH - normalize(DEFAULT_PARAMS.fppToxicThreshold, lane.max) * laneH : null;
        ...
        return (
          <g key={lane.key}>
            ...
            />
            <text x="20" y={y + 14} fontFamily={PAPER_THEME.tickFont} fontSize="10" fill={PAPER_THEME.tickColor}>
              {lane.label}
            </text>
      ...
      {[0, 25, 50, 75, 100].map((tick) => {
        ...
        return (
          <g key={tick}>
            <line
              x1={x}
              y1={plotTop + lanes.length * (laneH + laneGap) - laneGap}
              x2={x}
              y2={plotTop + lanes.length * (laneH + laneGap) - laneGap + 6}
              stroke={PAPER_THEME.grid}
            />
            <text
              x={x}
              y={plotTop + lanes.length * (laneH + laneGap) - laneGap + 18}
              textAnchor="middle"
              fontFamily={PAPER_THEME.tickFont}
              fontSize="10"
              fill={PAPER_THEME.tickColor}
      ...
      <text
        x={PAD_X}
        y={plotTop + lanes.length * (laneH + laneGap) - laneGap + 30}
        fontFamily={PAPER_THEME.tickFont}
        fontSize="9"
        fill={PAPER_THEME.tickColor}
        fontStyle="italic"
  ...
  return (
    <SVGChartContainer W={W} H={H + 10} ariaLabel="Hill feedback curve" variant="paper" style={{ height: "132px" }}>
      ...
      <line
        ...
        x2={markerX}
        y2={H - PAD + 4}
        stroke={`${THEME.CORAL}80`}
        strokeWidth={1}
        strokeDasharray="3 3"
      />
      <circle
        cx={markerX}
        cy={H - PAD + 4 - (hillFeedback(Math.min(currentFPP, fppMax), hill) / hill.Vmax) * (H - PAD * 2 + 4)}
        r={3}
        fill={THEME.CORAL}
      />
      ...
      <text x={W / 2} y={H + 6} fontFamily={THEME.MONO} fontSize="10" textAnchor="middle" fill={LABEL}>
        FPP (μM)
      </text>
      <text
        x={10}
        ...
        textAnchor="middle"
        fill={LABEL}
        transform={`rotate(-90, 10, ${(PAD + H - PAD) / 2})`}
      >
        ADS
      </text>
      <text
      ...
      {result && (
        ...
        >
          {[
            ...
            {
              label: "μ (growth)",
              value: `${result.currentState.specificGrowthRate.toFixed(4)} h⁻¹`,
              color: THEME.LILAC,
            },
            {
              label: "μ_max drift",
              value: `${result.diagnostics.parameterDrift.muMax}%`,
              color:
                Math.abs(result.diagnostics.parameterDrift.muMax) > 10
                  ? "rgba(250,128,114,0.7)"
                  : "rgba(147,203,82,0.7)",
            },
            {
              label: "Model Fit",
              value: `R² ${result.diagnostics.modelFit.toFixed(3)}`,
              color: result.diagnostics.modelFit > 0.9 ? "rgba(147,203,82,0.7)" : "rgba(250,128,114,0.7)",
            },
          ].map((m, i) => (
            <div
              key={i}
            ...
            {result.forecast
              ...
              .map((f, i) => (
                ...
                >
                  ...
                  <div
                    style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xxs)", color: "rgba(255,255,255,0.25)" }}
                  >
...
export function BioreactorAnalyticsPanel() {
  ...
  const [result, setResult] = useState<import("../../../server/bioreactorAnalyticsEngine").BioreactorAnalytics | null>(
    null,
  );
  const [loading, setLoading] = useState(false);
        ...
        <>
          ...
          >
            {[
              ...
              {
                label: "R²",
                value: result.kinetics.r2.toString(),
                color: result.kinetics.r2 > 0.8 ? "rgba(147,203,82,0.7)" : "rgba(250,128,114,0.7)",
              },
            ].map((m, i) => (
