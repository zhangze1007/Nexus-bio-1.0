...
export function useDynConState() {
  ...
  const [spontaneousLossRate, setSpontaneousLossRate] = usePersistedState(
    "nexus-bio:dyncon:spontaneousLossRate",
    SPONTANEOUS_LOSS_RATE,
  );
  const [o2ConsumptionCoeff, setO2ConsumptionCoeff] = usePersistedState(
    "nexus-bio:dyncon:o2ConsumptionCoeff",
    O2_CONSUMPTION_COEFF,
  );
      ...
      trajectory.forEach((s, i) => {
        ...
        if (s.product > params.productToxicThreshold) {
          violations.push({
            ...
            variable: "Product",
            value: s.product,
            bound: `< ${params.productToxicThreshold} g/L`,
          });
        }
      });
  ...
  const trajectory =
    controlMode === "mpc" && mpcResult && mpcResult.trajectory.length > 0
      ? mpcResult.trajectory
      : pidSimulation.trajectory;
  const simError = controlMode === "mpc" && mpcResult ? (mpcSimulation?.simError ?? null) : pidSimulation.simError;
