...
import { divergingColor, LAYER_COLORS } from "./multiOHelpers";
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px" }}>
        <ScientificFigureFrame
          ...
          legend={[
            {
              label: "Status",
              value: fluxomicsResult ? "Complete" : "Not run",
              accent: fluxomicsResult ? THEME.MINT : THEME.DIM,
            },
            {
              label: "Bottlenecks",
              value: `${fluxomicsResult?.bottlenecks.filter((b) => b.isBottleneck).length ?? 0}`,
              accent: THEME.CORAL,
            },
          ]}
        >
          ...
          {!fluxomicsResult && !fluxomicsLoading && (
            ...
            >
              ...
              <br />
              The analysis identifies metabolic bottlenecks and computes carbon/oxygen/ATP efficiency metrics.
            </div>
          )}
            ...
            <>
              ...
              <div style={{ display: "flex", gap: "12px", marginBottom: "20px", flexWrap: "wrap" }}>
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "12px 16px", flex: "1 0 120px" }}>
                  ...
                  <span
                    style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-lg)", fontWeight: 700, color: THEME.SKY }}
                  >
                    {fluxomicsResult.correlations.length}
                  </span>
                </div>
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "12px 16px", flex: "1 0 120px" }}>
                  <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, display: "block" }}>
                  ...
                  <span
                    style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-lg)", fontWeight: 700, color: THEME.MINT }}
                  >
                    {fluxomicsResult.correlations.filter((c) => c.significant).length}
                  </span>
                </div>
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "12px 16px", flex: "1 0 120px" }}>
                  <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, display: "block" }}>
                  ...
                  <span
                    style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-lg)", fontWeight: 700, color: THEME.CORAL }}
                  >
                    {fluxomicsResult.bottlenecks.filter((b) => b.isBottleneck).length}
                  </span>
                </div>
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "12px 16px", flex: "1 0 120px" }}>
                  <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, display: "block" }}>
                  ...
                  >
                    {(fluxomicsResult.efficiency.carbonEfficiency * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {fluxomicsResult.bottlenecks.slice(0, 12).map((b) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, height: "5px", borderRadius: "3px", background: THEME.PANEL_INSET }}>
                        <div
                          style={{
                            ...
                            height: "100%",
                            borderRadius: "3px",
                            background: b.isBottleneck ? THEME.CORAL : THEME.SKY,
                          }}
                        />
                      </div>
                      <span
                      ...
                      {b.isBottleneck && (
                        <ConfidenceBadge value={1 - b.utilization} thresholds={{ high: 0.5, low: 0.2 }} />
                      )}
                    </div>
                  ))}
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <SVGChartContainer W={hmW} H={hmH} ariaLabel="Flux-expression correlation heatmap" variant="paper">
                      ...
                      <rect
                        x={hmPAD_L + genes.length * cellW + 8}
                        y={hmPAD_T}
                        width="8"
                        height={reactions.length * cellH - 2}
                        fill="url(#flux-corr-grad)"
                        rx="2"
                      />
                      <text
                        x={hmPAD_L + genes.length * cellW + 12}
                        y={hmPAD_T - 4}
                        fontFamily={THEME.MONO}
                        fontSize="9"
                        fill={LABEL}
                      ...
                      <text
                        x={hmPAD_L + genes.length * cellW + 12}
                        y={hmPAD_T + reactions.length * cellH + 10}
                        fontFamily={THEME.MONO}
                        fontSize="9"
                        fill={LABEL}
                      >
