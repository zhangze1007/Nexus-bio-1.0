...
export function VolcanoPlot({
  ...
  function xPos(fc: number) {
    return PAD + ((fc + fcMax) / (fcMax * 2)) * (W - PAD * 2);
  }
  function yPos(pv: number) {
    return H - PAD - (Math.min(Math.max(0, -Math.log10(Math.max(pv, 1e-5))), pvMax) / pvMax) * (H - PAD * 2);
  }
  ...
  return (
    <SVGChartContainer W={W} H={H} ariaLabel="Volcano plot" variant="paper">
      ...
      <line
        ...
        strokeDasharray="4 3"
      />
      <rect x={fcLineR} y={PAD} width={W - PAD - fcLineR} height={pvLine - PAD} fill="rgba(147,203,82,0.06)" />
      <rect x={PAD} y={PAD} width={fcLineL - PAD} height={pvLine - PAD} fill="rgba(250,128,114,0.06)" />
      {data.map((row) => {
        ...
        const color = sig ? (up ? "rgba(147,203,82,0.85)" : "rgba(250,128,114,0.85)") : PAPER_THEME.scatterStroke;
        return (
          <g key={row.id}>
            {isHighlighted && (
              <circle cx={xPos(fc)} cy={yPos(pv)} r={8} fill="none" stroke="rgba(255,139,31,0.9)" strokeWidth={1.4} />
            )}
            <circle cx={xPos(fc)} cy={yPos(pv)} r={isHighlighted ? 5.5 : sig ? 4 : 2.5} fill={color}>
              <title>
                {row.gene}: FC={fc.toFixed(2)}, p={pv.toFixed(4)}
              </title>
            </circle>
            {isHighlighted && (
              <text
