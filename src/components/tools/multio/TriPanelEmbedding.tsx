...
import { CLUSTER_PAL, divergingColor, LAYER_COLORS, pearsonR } from "./multiOHelpers";
import { VolcanoPlot } from "./VolcanoPlot";

/* ── Tri-Panel Embedding: PCA biplot + correlation heatmap + volcano ─ */

export function TriPanelEmbedding({
  embeddings,
  data,
  fcThreshold,
  pvThreshold,
  ...
  /** If provided, use these 2D coordinates instead of PCA for the left panel. */
  umapOverride?: number[][];
}) {
  // ── PCA Biplot (left) ──────────────────────────────────────────────
  const pcaW = 280,
  ...
  // When umapOverride is provided, use UMAP 2D coordinates instead of PCA scores
  const { pcaProjected, loadingArrows, isUMAP } = useMemo(() => {
    ...
    if (umapOverride && umapOverride.length > 0) {
      // UMAP mode: project UMAP 2D coordinates into the plot area
      const xs = umapOverride.map((u) => u[0] ?? 0);
      const ys = umapOverride.map((u) => u[1] ?? 0);
      ...
      const projected = visible.map((p) => {
        ...
        const ux = idx >= 0 ? (umapOverride[idx]?.[0] ?? 0) : 0;
        const uy = idx >= 0 ? (umapOverride[idx]?.[1] ?? 0) : 0;
        const normX = ((ux - xMin) / xRange) * 2 - 1;
        const normY = ((uy - yMin) / yRange) * 2 - 1;
        return {
          ...p,
          sx: pcaPAD + (normX * plotExtent + plotExtent),
          sy: pcaPAD + (plotExtent - normY * plotExtent),
        };
      });
      ...
      return {
        ...
        loadingArrows: {
          pc1Pct: "UMAP-1",
          pc2Pct: "UMAP-2",
          arrows: [] as { label: string; pc1: number; pc2: number; x: number; y: number }[],
        },
        isUMAP: true,
      };
    }
  ...
  }, [visible, pcaResult, geneIdxMap, umapOverride]);
  ...
  const hmW = 300,
    hmH = 320,
    hmPAD = { top: 60, left: 60, right: 20, bottom: 8 };
    ...
    <div style={{ display: "flex", gap: "12px", width: "100%", height: "100%", padding: "8px" }}>
      ...
      <div style={{ flex: "0 0 auto" }}>
        <SVGChartContainer
          W={pcaW}
          H={pcaH}
          ariaLabel="PCA Biplot"
          variant="paper"
          style={{ width: `${pcaW}px`, height: `${pcaH}px` }}
        >
          <text
            x={pcaW / 2}
            y={14}
          ...
          >
            {isUMAP ? "UMAP PROJECTION" : "PCA BIPLOT"}
          </text>
          <text
            x={pcaW / 2}
            y={pcaH - 4}
            ...
            fill={PAPER_THEME.tickColor}
          >
            {isUMAP ? "UMAP-1" : `PC1 (${loadingArrows.pc1Pct}% var)`}
          </text>
          <text
            x={8}
            y={pcaH / 2}
            ...
            fontSize={PAPER_THEME.tickSize}
            fill={PAPER_THEME.tickColor}
            transform={`rotate(-90,8,${pcaH / 2})`}
          >
            {isUMAP ? "UMAP-2" : `PC2 (${loadingArrows.pc2Pct}% var)`}
          </text>
          <line
            x1={pcaPAD}
            y1={pcaH - pcaPAD}
          ...
          {loadingArrows.arrows.map((a, i) => {
            ...
            const color =
              layerColorMap[(["transcriptomics", "proteomics", "metabolomics"] as OmicsLayer[])[i]] ?? PAPER_THEME.axis;
            return (
              <g key={a.label}>
                <line
                  x1={cx}
          ...
          {pcaProjected.map((p, i) => (
            <circle
              ...
              cx={p.sx}
              cy={p.sy}
              r={p.gene === highlightedGene ? 5.5 : 3.5}
              fill={layerColorMap[p.layer] ?? CLUSTER_PAL[0]}
              opacity={0.8}
            />
          ))}
          {/* Layer legend */}
          {(["transcriptomics", "proteomics", "metabolomics"] as OmicsLayer[]).map(
            (layer, i) =>
              activeLayers[layer] && (
      ...
      <div style={{ flex: "0 0 auto" }}>
        <SVGChartContainer
          ...
          variant="paper"
          style={{ width: `${hmW}px`, height: `${hmH}px` }}
        >
          <text
            x={hmW / 2}
            y={12}
