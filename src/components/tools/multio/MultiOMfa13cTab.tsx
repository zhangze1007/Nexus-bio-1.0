...
export function MultiOMfa13cTab(props: Mfa13cTabProps) {
  const { activeTab, mfa13cResult, mfa13cLoading, mfa13cMCResult, mfa13cMCTrials, setMfa13cMCTrials, handleRunMFA13C } =
    props;
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px" }}>
          <ScientificFigureFrame
            ...
            legend={[
              {
                label: "Status",
                value: mfa13cResult ? (mfa13cResult.converged ? "Converged" : "Not converged") : "Not run",
                accent: mfa13cResult?.converged ? THEME.MINT : THEME.CORAL,
              },
          ...
          >
            {!mfa13cResult && !mfa13cLoading && (
              ...
              >
                ...
                <br />
                The engine uses EMU decomposition, Levenberg-Marquardt optimization, and Monte Carlo sampling for
              ...
              <>
                ...
                <ResultSummaryPanel
                  metrics={[
                    ...
                    {
                      label: "χ²/dof",
                      value: mfa13cResult.fitQuality.toFixed(4),
                      accent: mfa13cResult.fitQuality < 1 ? THEME.MINT : THEME.CORAL,
                    },
                    { label: "Iterations", value: mfa13cResult.nIterations, accent: THEME.LILAC },
                    {
                      label: "Objective",
                      value: mfa13cResult.objectiveFlux.toFixed(3),
                      accent: THEME.APRICOT,
                      trend: mfa13cResult.converged ? "up" : "flat",
                    },
                  ]}
                />
                ...
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "14px", marginBottom: "16px" }}>
                  ...
                  {(() => {
                    ...
                    const maxFlux = Math.max(
                      ...fluxes.map((f) => Math.abs(f.flux)),
                      ...(mfa13cMCResult?.fluxCIs?.map((c) => Math.abs(c[1])) ?? [1]),
                      0.01,
                    );
                ...
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "14px", marginBottom: "16px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                    {mfa13cResult.fluxEstimates.map((f, i) => {
                      ...
                      const dirColor =
                        f.direction === "forward" ? THEME.SKY : f.direction === "reverse" ? THEME.CORAL : THEME.APRICOT;
                      return (
                        <div
                          key={f.reactionId}
                          style={{
                  ...
                  {(() => {
                    ...
                    return (
                      <SVGChartContainer
                        W={hmW}
                        H={hmH}
                        ariaLabel="Mass isotopomer distribution heatmap"
                        variant="paper"
                      >
                        {Array.from({ length: maxN + 1 }, (_, j) => (
                          <text
                            key={`mh-${j}`}
                            x={hmPAD_L + j * hmCellW + hmCellW / 2}
                            y={hmPAD_T - 8}
                            textAnchor="middle"
                            fontFamily={THEME.MONO}
                            fontSize="10"
                        ...
                        <rect
                          x={hmPAD_L + (maxN + 1) * hmCellW + 8}
                          y={hmPAD_T}
                          width="8"
                          height={mids.length * hmCellH - 2}
                          fill="url(#mid-grad)"
                          rx="2"
                        />
                        <text
                          x={hmPAD_L + (maxN + 1) * hmCellW + 12}
                          y={hmPAD_T - 4}
                          fontFamily={THEME.MONO}
                          fontSize="9"
                          fill={LABEL}
                        ...
                        <text
                          x={hmPAD_L + (maxN + 1) * hmCellW + 12}
                          y={hmPAD_T + mids.length * hmCellH + 10}
                          fontFamily={THEME.MONO}
                          fontSize="9"
                          fill={LABEL}
                        >
                ...
                <div style={{ ...GLASS, borderRadius: "var(--nb-radius-md)", padding: "14px" }}>
                  ...
                  <div style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, lineHeight: 1.5 }}>
                    EMU decomposition with Levenberg-Marquardt nonlinear least squares. {mfa13cMCTrials} Monte Carlo
                    samples for 95% confidence intervals. Label substrate: glucose (carbons 0,1,2 labeled). Objective
                    reaction: CS (citrate synthase).
                    {mfa13cResult.converged
                      ? " Fit quality (χ²/dof < 1.0) indicates good agreement between simulated and measured MIDs."
                      : " Optimization did not converge to a good fit — consider adjusting measured MID data or network topology."}
                  </div>
                </div>
              </>
            )}
