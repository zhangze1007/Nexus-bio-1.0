...
import type { MOFAResult, VAEPerturbationPrediction, VAETrainingResult } from "../../../services/MOIEngine";
import {
...
export function useMultiOState() {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const transcriptIdx = headers.findIndex(
        (h) => h === "transcript" || h === "mrna" || h === "rna" || h === "expression",
      );
  ...
  const { data: model, error: simError } = useMemo(() => {
    try {
      return { data: new OmicsFoundationModel(activeData), error: null as string | null };
    } catch (e) {
      return {
        data: new OmicsFoundationModel(activeData),
        error: e instanceof Error ? e.message : "Model init failed",
      };
    }
  }, []);
  ...
  const handleRunMOFA = useCallback(async () => {
    ...
    setMofaBackendWarning(null);
    const views: Record<string, number[][]> = {
      transcriptomics: activeData.map((r) => [r.transcript ?? 0]),
      proteomics: activeData.map((r) => [r.protein ?? 0]),
      metabolomics: activeData.map((r) => [r.metabolite ?? 0]),
    };

    if (useMOFA) {
      try {
        const response = await fetch("/api/mofa", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ views, nFactors: 10, nIterations: 1000 }),
        });
        if (!response.ok) throw new Error(`MOFA+ backend returned ${response.status}`);
        const data = await response.json();
        if (data.ok === false) throw new Error(data.error ?? "MOFA+ backend error");
        setMofaPlusResult({
        ...
        const backendMsg = err instanceof Error ? err.message : "MOFA+ backend unavailable";
        setMofaBackendWarning(`${backendMsg} -- falling back to local ALS.`);
        try {
          setMofaPlusResult(runMOFA({ views, nFactors: 5 }));
        } catch (localErr) {
          setMultioError(localErr instanceof Error ? localErr.message : "MOFA+ analysis failed");
        }
      }
    } else {
      try {
        ...
        setMultioError(err instanceof Error ? err.message : "MOFA+ analysis failed");
      }
    }
  ...
  const handleRunUMAP = useCallback(async () => {
    setUmapLoading(true);
    setUmapBackendWarning(null);
    const featureMatrix = activeData.map((r) => [r.transcript ?? 0, r.protein ?? 0, r.metabolite ?? 0]);
    ...
    if (useRealUMAP) {
      try {
        const response = await fetch("/api/umap", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ data: featureMatrix, nNeighbors: 15, minDist: 0.1, nComponents: 2 }),
        });
        if (!response.ok) throw new Error(`UMAP backend returned ${response.status}`);
        const data = await response.json();
        if (data.ok === false) throw new Error(data.error ?? "UMAP backend error");
        setUmapResult(data.embedding);
      } catch (err) {
        const backendMsg = err instanceof Error ? err.message : "UMAP backend unavailable";
        setUmapBackendWarning(`${backendMsg} -- falling back to local PCA projection.`);
        setUmapResult(null);
      }
    } else {
      setUmapResult(null);
  ...
  const handleRunFluxomics = useCallback(() => {
    ...
    try {
      const fluxEstimates = activeData.map((row, i) => ({
        ...
        flux:
          Math.abs(row.transcript ?? 0) * 0.5 + Math.abs(row.protein ?? 0) * 0.3 + Math.abs(row.metabolite ?? 0) * 0.2,
  ...
  const [mfa13cMCResult, setMfa13cMCResult] = useState<ReturnType<
    typeof import("../../../server/mfa13CEngine").monteCarloConfidenceIntervals
  > | null>(null);
  ...
  const workflowSteps: StepDef[] = useMemo(
    () => [
      { id: "input", label: "Data Input", status: "done", detail: `${activeData.length} genes` },
      {
        ...
        status: vaeLoading ? "active" : vaeResult ? "done" : "pending",
        detail: vaeResult ? `${vaeResult.latentDim}D` : undefined,
      },
      {
        id: "cluster",
        label: "Clustering",
        status: vaeResult ? "done" : "pending",
        detail: `${Object.keys(mofaResult.factors).length > 0 ? mofaResult.factors.length : "—"} clusters`,
      },
      {
        id: "integrate",
        label: "Integration",
        status: mofaResult.totalVarianceExplained > 0 ? "done" : "pending",
        detail: `${(mofaResult.totalVarianceExplained * 100).toFixed(0)}% var`,
      },
      {
        id: "predict",
        label: "Prediction",
        status: perturbResult ? "done" : "pending",
        detail: perturbResult
          ? `${perturbResult.predicted_yield_change_percent >= 0 ? "+" : ""}${perturbResult.predicted_yield_change_percent.toFixed(1)}%`
          : undefined,
      },
    ],
  ...
  /* Derived data */
  const filtered = useMemo(() => activeData.filter((r) => Math.abs(r.fold_change ?? 0) > 0), []);
  ...
  const layerSignals = useMemo(() => {
    ...
    bottleneck.layer_signals.forEach((h) => {
      acc[h.layer] += h.weight;
    });
    return acc;
  }, [bottleneck]);
        ...
        evidence: [
          {
            ...
            source: "computation",
            reference:
              "Deterministic local computation: linear factor decomposition, linear projection, and sensitivity-style perturbation.",
            confidence: "demo",
