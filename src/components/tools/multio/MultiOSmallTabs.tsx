...
import type { MOFAResult, VAETrainingResult, MetabolicEfficiencyScore } from '../../../services/MOIEngine';
...
const { label: LABEL, value: VALUE, glass: GLASS, border: BORDER, inputBg: INPUT_BG, inputBorder: INPUT_BORDER, inputText: INPUT_TEXT } = toolTokens;
...
export function MultiOVolcanoTab(props: VolcanoTabProps) {
  const {
    activeTab, filtered, selectedGene, setSelectedGene, geneNames,
...
interface FactorsTabProps {
  ...
  scspatialPayload?: { result?: { clusterSummaries?: Array<{ clusterId: number; clusterLabel: string; cellCount: number; meanExpression: number; fate: string; topGenes: string[] }> } };
}
