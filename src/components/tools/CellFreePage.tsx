  ...
  return (
    ...
    >
      ...
      <ToolTabPanel tabId="metabolic" activeId={activeTab}>
        <CellFreeMetabolicPanel />
      </ToolTabPanel>
    </ToolShell>
  );
});

/* ── Cell-Free Metabolic Panel ──────────────────────────────────────────── */

function CellFreeMetabolicPanel() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{
        ...
        <select value={energySystem} onChange={(e) => setEnergySystem(e.target.value as typeof energySystem)}
      ...
      {result && (
        <>
          <div style={{ background: THEME.PANEL_SURFACE, borderRadius: 'var(--nb-radius-lg)', padding: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8, border: `1px solid ${THEME.BORDER}` }}>
            {[
              ...
              { label: 'ATP Produced', value: `${result.energyBalance.atpProduced.toFixed(1)} mM`, color: THEME.APRICOT },
              { label: 'ATP Net', value: `${result.energyBalance.net.toFixed(1)} mM`, color: result.energyBalance.net > 0 ? 'rgba(147,203,82,0.7)' : 'rgba(250,128,114,0.7)' },
          ...
          {result.recommendations.length > 0 && (
            <div style={{ background: THEME.PANEL_SURFACE, borderRadius: 'var(--nb-radius-lg)', padding: 12, border: `1px solid ${THEME.BORDER}` }}>
              ...
              {result.recommendations.map((r, i) => (
                <div key={i} style={{ fontFamily: THEME.SANS, fontSize: 'var(--nb-fs-sm)', color: 'rgba(255,255,255,0.7)', marginBottom: 4 }}>• {r}</div>
              ))}
            </div>
          )}
        </>
