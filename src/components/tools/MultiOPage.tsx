...
import { MultiOEfficiencyTab, MultiOFactorsTab, MultiOProjectionTab, MultiOVolcanoTab } from "./multio/MultiOSmallTabs";
import { LAYER_COLORS, MULTIO_TABS } from "./multio/multiOHelpers";
    ...
    <ToolShell
          ...
          aside={
            <>
              ...
              <div style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.VALUE, fontWeight: 700 }}>
                {MULTIO_TABS.find((t) => t.id === s.activeTab)?.label ?? "Embedding"} ·{" "}
                {Object.values(s.activeLayers).filter(Boolean).length}/3 omics layers active
              </div>
              <div
                style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.LABEL, lineHeight: 1.55 }}
              >
                The current lens is anchored to{" "}
                {s.analyzeArtifact?.targetProduct ??
                  s.project?.targetProduct ??
                  s.project?.title ??
                  "the active project object"}
                , so bottleneck claims stay attached to the same scientific context.
              </div>
            </>
          }
          signals={[
            ...
            },
            {
              ...
              detail:
                "Efficiency scores let omics interpretation stay tied to production relevance, not just statistical significance.",
              tone: "neutral",
            },
          ]}
        />
      ...
      footer={
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <DataSourceBadge
            source={s.dataSource === "uploaded" ? "live" : "mock"}
            label={s.dataSource === "uploaded" ? "User CSV" : "Demo Data"}
          />
          <input
            ref={s.fileInputRef}
            type="file"
          ...
          <ExportButton
            label="Export Significant JSON"
            data={s.significant}
            filename="multio-significant"
            format="json"
          />
        </div>
      }
    >
      {s.simError && (
        <div style={{ padding: "0 16px 8px" }}>
          <SimErrorBanner message={s.simError} />
        </div>
      )}
      {s.multioError && (
        <div style={{ padding: "0 16px 8px" }}>
          <SimErrorBanner message={s.multioError} onRetry={() => s.setMultioError(null)} />
        </div>
      )}
      ...
      <div style={{ padding: "0 16px 4px" }}>
        <WorkflowStepper
          ...
          activeIndex={s.workflowSteps.findIndex((st) => st.status === "active")}
        />
      </div>
      ...
      <div style={{ padding: "0 16px 8px" }}>
        <ResultSummaryPanel
          metrics={[
            ...
            {
              label: "Var. Explained",
              value: `${(s.mofaResult.totalVarianceExplained * 100).toFixed(1)}%`,
              unit: "",
              accent: THEME.MINT,
            },
            {
              ...
              trend: s.significant.length > 10 ? "up" : "flat",
            },
          ]}
        />
      </div>
      ...
      <MultiOEmbeddingTab
        ...
        vaeError={s.vaeError}
        useRealUMAP={s.useRealUMAP}
        setUseRealUMAP={s.setUseRealUMAP}
        umapResult={s.umapResult}
        umapLoading={s.umapLoading}
        handleRunUMAP={s.handleRunUMAP}
        umapBackendWarning={s.umapBackendWarning}
      />
      ...
      {/* ── Factors Tab ── */}
      <MultiOFactorsTab activeTab={s.activeTab} mofaResult={s.mofaResult} scspatialPayload={s.scspatialPayload} />
      ...
      {/* ── Efficiency Tab ── */}
      <MultiOEfficiencyTab activeTab={s.activeTab} efficiencyScores={s.efficiencyScores} />
