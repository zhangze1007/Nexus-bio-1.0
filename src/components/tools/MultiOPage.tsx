...
export default React.memo(function MultiOPage() {
  ...
  const handleRunFluxomics = useCallback(() => {
    ...
    try {
      // Build mock 13C-MFA flux estimates from omics data
      ...
      // Build gene expression from transcript layer
  ...
  const handleRunMFA13C = useCallback(async () => {
    ...
    try {
      ...
      // Build demo metabolic network from omics data
      ...
      // Build measured MIDs from active omics data (use first few genes as proxy)
      ...
      for (let i = 0; i < nMet; i++) {
        ...
        // Distribute into MID format (nCarbon + 1 entries)
        ...
        // Fill remaining with small uniform probability
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '0 16px 4px' }}>
        <WorkflowStepper steps={workflowSteps} activeIndex={workflowSteps.findIndex(s => s.status === 'active')} />
      </div>
      ...
      <div style={{ padding: '0 16px 8px' }}>
        <ResultSummaryPanel
          metrics={[
            { label: 'Samples', value: activeData.length, accent: THEME.SKY },
            { label: 'Clusters', value: mofaResult.factors.length, accent: THEME.LILAC },
            { label: 'Var. Explained', value: `${(mofaResult.totalVarianceExplained * 100).toFixed(1)}%`, unit: '', accent: THEME.MINT },
            { label: 'Significant', value: significant.length, accent: THEME.APRICOT, trend: significant.length > 10 ? 'up' : 'flat' },
          ]}
        />
      </div>
      ...
      {/* ── ML Predict Tab ──────────────────────────────────────────────────── */}
      <ToolTabPanel tabId="mlpredict" activeId={activeTab}>
        <MLPredictPanel />
      </ToolTabPanel>
              ...
              <>
                ...
                <div style={{ ...GLASS, borderRadius: 'var(--nb-radius-md)', padding: '14px', marginBottom: '16px' }}>
                  ...
                  {(() => {
                    ...
                    const PAD_L = 100, PAD_T = 28;
                    const reactions = [...new Set(topCorrs.map(c => c.reactionId))];
                    const genes = [...new Set(topCorrs.map(c => c.geneId))];
                    const hmW = PAD_L + genes.length * cellW + 60;
                    const hmH = PAD_T + reactions.length * cellH + 10;
                    ...
                    return (
                      <SVGChartContainer W={hmW} H={hmH} ariaLabel="Flux-expression correlation heatmap" variant="paper">
                        ...
                        {genes.map((g, gi) => (
                          <text key={`gl-${gi}`}
                            x={PAD_L + gi * cellW + cellW / 2} y={PAD_T - 8}
                            textAnchor="middle" fontFamily={THEME.MONO} fontSize="10" fill={LABEL}>
                            {g.replace('R_', '').slice(0, 6)}
                          </text>
                        ))}
                        {/* Reaction labels + cells */}
                            ...
                            <g key={rxn}>
                              ...
                              {genes.map((gene, gi) => {
                                ...
                                return (
                                  <g key={`${rxn}-${gene}`}>
                                    <rect
                                      x={PAD_L + gi * cellW} y={y}
                                      width={cellW - 2} height={cellH - 2}
                                      fill={divergingColor(r)}
                                      rx={3}
                                    />
                                    <text
                                      x={PAD_L + gi * cellW + cellW / 2 - 1}
                                      y={y + cellH * 0.6}
                                      textAnchor="middle" fontFamily={THEME.MONO} fontSize="9"
                        ...
                        <rect x={PAD_L + genes.length * cellW + 8} y={PAD_T} width="8" height={reactions.length * cellH - 2}
                          fill="url(#flux-corr-grad)" rx="2" />
                        <text x={PAD_L + genes.length * cellW + 12} y={PAD_T - 4} fontFamily={THEME.MONO} fontSize="9" fill={LABEL}>+1</text>
                        <text x={PAD_L + genes.length * cellW + 12} y={PAD_T + reactions.length * cellH + 10} fontFamily={THEME.MONO} fontSize="9" fill={LABEL}>-1</text>
                      </SVGChartContainer>
                    );
                  })()}
                </div>
                ...
                <>
                  ...
                  <div style={{ ...GLASS, borderRadius: 'var(--nb-radius-md)', padding: '14px', marginBottom: '16px' }}>
                    ...
                    {(() => {
                      ...
                      const PAD_L = 100, PAD_T = 28;
                      const hmW = PAD_L + (maxN + 1) * cellW + 60;
                      const hmH = PAD_T + mids.length * cellH + 10;
                      return (
                        <SVGChartContainer W={hmW} H={hmH} ariaLabel="Mass isotopomer distribution heatmap" variant="paper">
                          {/* Column headers (M+0, M+1, ...) */}
                          {Array.from({ length: maxN + 1 }, (_, j) => (
                            <text key={`mh-${j}`}
                              x={PAD_L + j * cellW + cellW / 2} y={PAD_T - 8}
                              textAnchor="middle" fontFamily={THEME.MONO} fontSize="10" fill={LABEL}>
                              M+{j}
                            </text>
                          ))}
                          ...
                          <rect x={PAD_L + (maxN + 1) * cellW + 8} y={PAD_T} width="8" height={mids.length * cellH - 2}
                            fill="url(#mid-grad)" rx="2" />
                          <text x={PAD_L + (maxN + 1) * cellW + 12} y={PAD_T - 4} fontFamily={THEME.MONO} fontSize="9" fill={LABEL}>Max</text>
                          <text x={PAD_L + (maxN + 1) * cellW + 12} y={PAD_T + mids.length * cellH + 10} fontFamily={THEME.MONO} fontSize="9" fill={LABEL}>0</text>
                        </SVGChartContainer>
                      );
                    })()}
                  </div>
...
function MLPredictPanel() {
  ...
  const handlePredict = useCallback(async () => {
    ...
    try {
      ...
      const expressions = geneExpr.split(',').reduce((acc, v, i) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16 }}>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xxs)', color: 'rgba(255,255,255,0.4)' }}>Protein Sequence</div>
            <input value={sequence} onChange={(e) => setSequence(e.target.value)}
              style={{ width: '100%', padding: '4px 8px', background: THEME.INPUT_BG, border: `1px solid ${THEME.INPUT_BORDER}`, borderRadius: 'var(--nb-radius-sm)', color: THEME.INPUT_TEXT, fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', outline: 'none' }}
            />
          </div>
          <div>
            <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xxs)', color: 'rgba(255,255,255,0.4)' }}>Gene Expression (csv)</div>
            <input value={geneExpr} onChange={(e) => setGeneExpr(e.target.value)}
              style={{ width: 150, padding: '4px 8px', background: THEME.INPUT_BG, border: `1px solid ${THEME.INPUT_BORDER}`, borderRadius: 'var(--nb-radius-sm)', color: THEME.INPUT_TEXT, fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', outline: 'none' }}
            />
          </div>
        </div>
        <button onClick={handlePredict} disabled={loading} className="nb-tool-toggle"
      ...
      {enzymeResult && (
        <div style={{ background: THEME.PANEL_SURFACE, borderRadius: 'var(--nb-radius-lg)', padding: 12, border: `1px solid ${THEME.BORDER}` }}>
          <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: THEME.LABEL, marginBottom: 6 }}>Enzyme Function (PROSITE signatures + ESM-2)</div>
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xxs)', color: THEME.LABEL }}>EC Class</div>
              <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-lg)', color: THEME.LILAC, fontWeight: 700 }}>{enzymeResult.predictedEC}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
      ...
      {fluxResult && (
        <div style={{ background: THEME.PANEL_SURFACE, borderRadius: 'var(--nb-radius-lg)', padding: 12, border: `1px solid ${THEME.BORDER}` }}>
          <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: THEME.LABEL, marginBottom: 6 }}>Flux Prediction (Monod kinetics)</div>
          <div style={{ display: 'flex', gap: 12 }}>
            {Object.entries(fluxResult.predictedFluxes).map(([rxn, flux]) => (
              <div key={rxn} style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-sm)', color: THEME.APRICOT }}>{(flux as number).toFixed(3)}</div>
              </div>
            ))}
          </div>
        </div>
