...
import { CycleProgressRing, PHASE_PASTEL, PHASES, Timeline } from "./sharedComponents";
    ...
    >
      <ScientificFigureFrame
        ...
        legend={[
          ...
          {
            label: "Feedback",
            value: hasCommittedFeedback ? "Committed" : "Draft only",
            accent: hasCommittedFeedback ? THEME.sky : THEME.coral,
          },
        ]}
        footer={
          <div style={{ display: "grid", gap: "6px" }}>
            <div
              style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.paperValue, lineHeight: 1.55 }}
            >
              The central panel now behaves like an experimental ledger figure. Phase state, campaign trajectory, and
              governance status stay in one reading path so loop health can be judged at a glance.
            </div>
            <div style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.paperLabel }}>
              latest iteration #{latestIteration?.id ?? "—"} · {latestIteration?.result ?? "—"}{" "}
              {latestIteration?.unit ?? ""} · feedback{" "}
              {hasCommittedFeedback ? "requires approved delta" : "still locked"}
            </div>
          </div>
        }
        minHeight="100%"
