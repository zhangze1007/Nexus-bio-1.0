...
import type { DBTLIteration, DBTLPhase, FeedbackLoopResult } from "../../../types";
...
export function sourceProvenanceIdsFromFeedback(feedback: DBTLLearnedFeedback): string[] {
  return uniqueStrings(
    feedback.sources.flatMap((source) => (source.provenanceEntryId ? [source.provenanceEntryId] : [])),
  );
}
  ...
  return (
    ...
    >
      ...
      <rect
        ...
        y="18"
        width="484"
        height={Math.max(320, iterations.length * 60)}
        rx="14"
        fill="rgba(255,255,255,0.02)"
        stroke="rgba(255,255,255,0.06)"
      />
      <text x="36" y="14" fontFamily={THEME.SANS} fontSize="10" fill={THEME.paperLabel} letterSpacing="0.12em">
        DBTL AUDIT TIMELINE
      </text>
      <text x="36" y="28" fontFamily={THEME.SANS} fontSize="11" fill={THEME.paperValue}>
        Iteration trace with phase identity, result magnitude, and pass gate
      </text>
      {iterations.length > 1 && (
        <polyline
          points={iterations.map((it, i) => `${160 + (it.result / maxResult) * 280},${30 + i * 60 + 33}`).join(" ")}
          fill="none"
          stroke="#FF7F00"
          strokeWidth={1.8}
          strokeDasharray="4 2"
      ...
      <line
        x1={160 + (targetThreshold / maxResult) * 280}
        y1={20}
        x2={160 + (targetThreshold / maxResult) * 280}
        y2={30 + iterations.length * 60}
        stroke="rgba(255,139,31,0.24)"
        strokeDasharray="5 4"
      />
      {iterations.map((it, i) => {
        const y = 30 + i * 60;
        ...
        return (
          <g key={it.id}>
            ...
            <text x={100} y={y + 20} fontFamily={THEME.SANS} fontSize="10" fill="rgba(255,255,255,0.5)">
              {it.hypothesis.slice(0, 40)}
              {it.hypothesis.length > 40 ? "…" : ""}
            </text>
            <rect
              x={160}
              y={y + 28}
      ...
      <text
        x={160 + (targetThreshold / maxResult) * 280 + 4}
        y={18}
        fontFamily={THEME.MONO}
        fontSize="10"
        fill="rgba(255,139,31,0.78)"
      >
        target band
      </text>
      <text
        x={160}
        y={30 + iterations.length * 60 + 16}
        fontFamily={THEME.MONO}
        fontSize="10"
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "16px 0" }}>
      <svg role="img" aria-label="Chart" width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        {/* Track */}
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={THEME.progressTrack} strokeWidth={stroke} />
        {/* Progress arc */}
        <circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
          fill="none"
          stroke="url(#pathd-progress-ring)"
          strokeWidth={stroke}
          strokeLinecap="round"
      ...
      >
        ...
        <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: THEME.paperLabel }}>iterations</span>
      </div>
    </div>
  );
}
