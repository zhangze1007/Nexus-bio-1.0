...
import type { CFSFullResult, CFSParameters, GeneConstruct } from "../../../services/CellFreeEngine";
...
import { ChartGrid, ChartLegend, SVGChartContainer } from "../../charts/primitives";
...
export function TimeCourseChart({ result, constructs }: { result: CFSFullResult; constructs: GeneConstruct[] }) {
  ...
  const radarScores = useMemo(() => {
    ...
    return sim.genes.map((g) => {
      ...
      const stability =
        1 - (Math.max(...g.protein) - g.protein[g.protein.length - 1]) / (Math.max(...g.protein) + 0.001);
      ...
      return {
        geneId: g.geneId,
        geneName: g.geneName,
        scores: [efficiency, Math.max(0, Math.min(1, stability)), rate, yieldPerATP, repro],
      };
    });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "8px", width: "100%" }}>
      ...
      <svg role="img" aria-label="ODE time course" viewBox={`0 0 ${TOP_W} ${TOP_H}`} style={{ width: "100%" }}>
        ...
        <text
          ...
          fontSize={PAPER_THEME.tickSize}
          fill={PAPER_THEME.tickColor}
          transform={`rotate(-90,12,${TOP_H / 2})`}
        >
          Expression (a.u.)
        </text>
        ...
        {sim.genes.map((g, gi) => {
          ...
          const areaD =
            catmullRomPath(pts) +
            ` L ${pts[pts.length - 1][0].toFixed(1)} ${BASE_Y} L ${pts[0][0].toFixed(1)} ${BASE_Y} Z`;
          return (
            <g key={g.geneId}>
              <motion.path
                d={areaD}
              ...
              <motion.path
                ...
                transition={{ duration: 1.2, ease: "easeOut", delay: gi * 0.15 }}
              />
            </g>
          );
        })}
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <svg role="img" aria-label="Resource depletion" viewBox={`0 0 ${BL_W} ${BL_H}`} style={{ flex: 1 }}>
          ...
          ).map(({ key, initV, color }, si) => {
            const pts: [number, number][] = res.time.map((t, i) => [bsx(t), bsy(Math.min(1, res[key][i] / initV))]);
            const areaD =
              catmullRomPath(pts) +
              ` L ${pts[pts.length - 1][0].toFixed(1)} ${bsy(0)} L ${pts[0][0].toFixed(1)} ${bsy(0)} Z`;
            return (
              <g key={key}>
                <motion.path
                  d={areaD}
                  fill={color}
                  opacity={0.25 + si * 0.05}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 0.25 + si * 0.05 }}
          ...
          {[
            ["ATP", RES_COLORS.atp],
            ["Ribosomes", RES_COLORS.rib],
            ["Amino acids", RES_COLORS.aa],
          ].map(([label, col], i) => (
        ...
        <svg role="img" aria-label="Construct radar chart" viewBox={`0 0 ${BR_W} ${BR_H}`} style={{ flex: 1 }}>
          ...
          {[0.25, 0.5, 0.75, 1].map((scale) => (
            <polygon
              key={scale}
              points={AXES.map((_, axis) => {
                const [x, y] = radarPt(scale, axis);
...
export function ResourceChart({ result }: { result: CFSFullResult }) {
  ...
  function sx(t: number) {
    return PAD + (t / tMax) * (W - PAD * 2);
  }
  function sy(f: number) {
      ...
      {depTime > 0 && depTime < tMax && (
        <>
          <line
            ...
            x2={sx(depTime)}
            y2={H - PAD}
            stroke={`rgba(${SEMANTIC_RGB.fail}, 0.5)`}
            strokeWidth={1}
            strokeDasharray="4,3"
          />
          <text
            ...
            fontFamily={PAPER_THEME.tickFont}
            fontSize={PAPER_THEME.tickSize}
            fill={`rgba(${SEMANTIC_RGB.fail}, 0.78)`}
          >
            Depletion
          </text>
        </>
...
export function FittingChart({ result }: { result: CFSFullResult }) {
  ...
  function sx(c: number) {
    return PAD + (c / cMax) * (W - PAD * 2);
  }
  function sy(r: number) {
    return PAD + (1 - r / rMax) * (mainH - PAD * 2);
  }
    ...
    <SVGChartContainer W={W} H={H} ariaLabel="Fitting curve with residuals" variant="paper">
      ...
      {curve
        ...
        .map((p, i) => (
          <g key={`dp${i}`}>
            <line
              x1={sx(p.concentration)}
              y1={sy(p.rate * 0.9)}
              x2={sx(p.concentration)}
              y2={sy(p.rate * 1.1)}
              stroke={`${THEME.MINT}66`}
              strokeWidth={1}
            />
            <circle
              cx={sx(p.concentration)}
              cy={sy(p.rate)}
              r={3}
              fill={`${THEME.MINT}CC`}
              stroke={`${THEME.MINT}66`}
      ...
      <text
        ...
        fontFamily={PAPER_THEME.tickFont}
        fontSize={PAPER_THEME.tickSize}
        fill={`${THEME.APRICOT}E6`}
      >
        Vmax={fit.vmax.toFixed(2)}
      </text>
      <text
        x={PAD + 8}
        y={PAD + 14}
      ...
      <line
        ...
        strokeWidth={0.5}
      />
      <line x1={PAD} y1={resTop} x2={PAD} y2={resTop + resH} stroke={PAPER_THEME.grid} strokeWidth={0.5} />
...
export function IvIvChart({ result }: { result: CFSFullResult }) {
  ...
  return (
    <SVGChartContainer W={W} H={H} ariaLabel="In-vitro to in-vivo translation" variant="paper">
      ...
      <text
        x={PAD + 40 + barW / 2}
        y={barBaseY - barH(invitro) - 6}
        textAnchor="middle"
        fontFamily={PAPER_THEME.tickFont}
        fontSize={PAPER_THEME.tickSize}
        fill={PAPER_THEME.titleColor}
      >
        {invitro.toFixed(1)} nM
      </text>

      <rect
        x={PAD + 40 + barW + barGap}
        y={barBaseY - barH(invivo)}
        width={barW}
        height={barH(invivo)}
        fill={THEME.MINT}
        rx={4}
      ...
      <text
        x={PAD + 40 + barW + barGap + barW / 2}
        y={barBaseY - barH(invivo) - 6}
        textAnchor="middle"
        fontFamily={PAPER_THEME.tickFont}
        fontSize={PAPER_THEME.tickSize}
        fill={PAPER_THEME.titleColor}
      >
        {getIvivExpressionLabel(invivo)}
      </text>

      <line
        x1={PAD + 20}
        y1={barBaseY}
        x2={PAD + 40 + barW * 2 + barGap + 20}
        y2={barBaseY}
        stroke={PAPER_THEME.axis}
        strokeWidth={0.5}
      />
        ...
        return (
          <g key={`cf${i}`}>
            ...
            <rect
              ...
              width={bw}
              height={8}
              fill={positive ? `${THEME.MINT}80` : `${THEME.CORAL}80`}
              rx={2}
            />
            <text
              x={corrLeft + bw + 4}
    ...
    >
      <svg role="img" aria-label="Chart" viewBox="0 0 720 420" style={{ width: "100%", height: "100%" }}>
        ...
        <text
          ...
          fill={PAPER_THEME.titleColor}
        >
          Resource state, construct yield, and IVIV translation in one reactor-facing schematic
        </text>
        ...
        <rect
          x="66"
          y={vesselTop + reactorHeight - fillHeight - 12}
          width="132"
          height={fillHeight}
          rx="18"
          fill={`rgba(${SEMANTIC_RGB.pass}, 0.32)`}
          stroke={`rgba(${SEMANTIC_RGB.pass}, 0.48)`}
        />
        <line
          x1="66"
          y1={vesselTop + reactorHeight - fillHeight - 12}
          x2="198"
          y2={vesselTop + reactorHeight - fillHeight - 12}
          stroke={`rgba(${SEMANTIC_RGB.pass}, 0.85)`}
          strokeDasharray="4 3"
        />
        <text
          x="76"
          y={vesselTop + reactorHeight + 24}
          fontFamily={PAPER_THEME.tickFont}
          fontSize={PAPER_THEME.tickSize}
          fill={PAPER_THEME.labelColor}
        >
        ...
        <text
          x="76"
          y={vesselTop + reactorHeight + 38}
          fontFamily={PAPER_THEME.tickFont}
          fontSize={PAPER_THEME.tickSize}
          fill={PAPER_THEME.titleColor}
        >
          {(depletionRatio * 100).toFixed(0)}% energy-support window
        </text>
          ...
          return (
            <g key={construct.id}>
              <rect
                ...
                height={height}
                rx="10"
                fill={GENE_COLORS[index % GENE_COLORS.length]}
                opacity="0.86"
              />
              ...
              <text
                ...
                fill={PAPER_THEME.titleColor}
              >
                {steady ? steady.maxProtein.toFixed(1) : "0.0"}
              </text>
            </g>
          );
        })}
        ...
        <text
          ...
          fill={PAPER_THEME.titleColor}
        >
          {result.simulation.totalProteinYield.toFixed(1)} nM in vitro total yield ·{" "}
          {result.simulation.energyDepletionTime.toFixed(0)} min depletion horizon
        </text>
        <text
          x="264"
          y="122"
      ...
      <div style={{ position: "absolute", top: "10px", right: "12px", width: "min(260px, calc(100% - 24px))" }}>
        ...
        >
          ...
          >
            Reactor 3D binds the simulated TX-TL state to one scene: depletion timing drives tank fill, gene yield
            drives tower height, and ATP/GTP/PEP are kept visible as explicit resource assumptions.
          </p>
          <div style={{ display: "grid", gap: "6px" }}>
            ...
            >
              energy pool ·{" "}
              {(params.initialEnergy.atp + params.initialEnergy.gtp + params.initialEnergy.pep).toFixed(1)} mM
            </span>
          </div>
        </div>
      </div>
