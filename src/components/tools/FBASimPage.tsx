  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
        ...
        <span style={{ fontFamily: MONO, fontSize: '11px', color: 'rgba(255,255,255,0.75)' }}>
          ...
          <div style={{ width: '240px', flexShrink: 0, overflowY: 'auto', padding: '16px', borderRight: '1px solid rgba(255,255,255,0.06)', background: '#0a0c10' }}>
            ...
            {(['biomass', 'atp', 'product'] as const).map(opt => (
              <button key={opt} onClick={() => setObjective(opt)} style={{
                ...
                background: objective === opt ? 'rgba(255,255,255,0.08)' : 'transparent',
                border: `1px solid ${objective === opt ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)'}`,
                borderRadius: '3px',
                color: objective === opt ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.35)',
            ...
            {Object.entries(result.shadowPrices).map(([met, price]) => (
              <div key={met} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <span style={{ fontFamily: MONO, fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>{met}</span>
                <span style={{ fontFamily: MONO, fontSize: '10px', color: price < 0 ? 'rgba(120,200,255,0.8)' : 'rgba(255,200,80,0.8)' }}>
                  {price.toFixed(3)}
                </span>
              </div>
            ))}
            ...
            {top5.map((r, i) => (
              <div key={r.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontFamily: MONO, fontSize: '10px', color: 'rgba(120,220,180,0.9)' }}>
                    {(r.flux ?? 0).toFixed(2)}
                  </span>
                </div>
                <div style={{ fontFamily: SANS, fontSize: '10px', color: 'rgba(255,255,255,0.25)', marginTop: '2px' }}>{r.name}</div>
                {/* Mini flux bar */}
