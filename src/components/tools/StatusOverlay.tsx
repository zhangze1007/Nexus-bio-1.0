...
import type { MachineState, SimParams, SimReadouts } from "../../machines/metabolicMachine";
import { michaelisRate, STATE_LABELS } from "../../machines/metabolicMachine";
...
function Sparkline({ data, height = 36 }: { data: number[]; height?: number }) {
  ...
  const lastX = w,
    lastY = h - ((data[data.length - 1] - min) / range) * (h - 4) - 2;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height, display: "block" }}>
      ...
      <polyline points={`0,${h} ${pts} ${w},${h}`} fill="url(#spark-grad)" stroke="none" />
      <polyline
        points={pts}
        fill="none"
        stroke={sparkColor}
        ...
        >
          <div
            style={{
              ...
              fontWeight: 700,
              color: opacity > 0.7 ? THEME.VALUE : THEME.LABEL,
              fontVariantNumeric: "tabular-nums",
            }}
          >
            {v.toFixed(1)}
  ...
  return (
    <div style={{ marginTop: "6px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
        ...
        <span style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.VALUE, fontWeight: 700 }}>
          {pct.toFixed(0)}%
        </span>
      </div>
      <div
        ...
        {state === "equilibrium" && (
          <motion.div
            ...
            style={{
              ...
              padding: "8px 10px",
              borderRadius: "var(--nb-radius-md)",
              background: "rgba(191,220,205,0.14)",
              border: "1px solid rgba(191,220,205,0.28)",
              textAlign: "center",
            }}
          >
            <span
