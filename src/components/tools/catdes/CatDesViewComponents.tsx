...
import { BORDER, dataCell, hdrCell, LABEL, PHASE_COLORS, tn, VALIDITY_STYLES, VALUE } from "./catdesShared";
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "baseline" }}>
        ...
        <div>
          ...
          <p style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-md)", color: VALUE, margin: "1px 0 0", ...tn }}>
            {result.predictedKd.toFixed(2)} <span style={{ fontSize: "var(--nb-fs-xs)", color: LABEL }}>μM</span>
          </p>
        </div>
        <div>
          <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL }}>ΔG</span>
          <p style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-md)", color: VALUE, margin: "1px 0 0", ...tn }}>
            {result.bindingEnergy.toFixed(2)}{" "}
            <span style={{ fontSize: "var(--nb-fs-xs)", color: LABEL }}>kcal/mol</span>
          </p>
        </div>
      </div>
        ...
        <div style={{ marginTop: 4, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3px 12px" }}>
          {axes.map((ax) => (
            <div key={ax.label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, minWidth: 42 }}>
                {ax.label}
              </span>
            ...
            {enzyme.catalyticResidues.map((r) => {
              ...
              return (
                <tr key={r.position} style={{ background: "rgba(255,255,255,0.015)" }}>
                  ...
                  <td style={{ ...dataCell, textAlign: "left", fontSize: "var(--nb-fs-xs)", color: LABEL }}>
                    {r.role.replace("_", " ")}
                  </td>
...
export function SequenceView({ result }: { result: SequenceDesignResult }) {
  const caiColor = (v: number) => (v >= 0.75 ? THEME.MINT : v >= 0.55 ? THEME.RISK_LOW : "rgba(255,120,120,0.7)");
    ...
    <div style={{ padding: "10px 12px" }}>
        ...
        <tbody>
          {result.designs.map((d) => (
            <tr key={d.rank} style={{ background: "rgba(255,255,255,0.015)" }}>
              <td style={{ ...dataCell, textAlign: "left", color: PHASE_COLORS.sequence, fontWeight: 600 }}>
                #{d.rank}
              </td>
      ...
      {result.consensusMotifs.length > 0 && (
        <div style={{ marginTop: 8, display: "flex", gap: 4, flexWrap: "wrap" }}>
          <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, marginRight: 4 }}>
            Motifs:
          </span>
...
export function FluxCostView({ result }: { result: MetabolicDrainResult }) {
  const viabilityColor = result.isViable ? (result.growthPenalty < 10 ? THEME.MINT : THEME.RISK_LOW) : THEME.CORAL;
        ...
        <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 4 }}>
          {[
            ...
            <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: LABEL, minWidth: 52 }}>
                {item.label}
              </span>
              <MiniBar
                value={item.value}
                color={item.color}
                max={Math.max(result.atpCost, result.nadphCost, result.ribosomeBurden) || 1}
              />
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: "10px 12px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <StatusDot
          color={result.isBalanced ? THEME.MINT : THEME.CORAL}
          label={result.isBalanced ? "Balanced" : "Imbalanced"}
        />
      </div>
      ...
      {result.convergenceHistory.length > 1 &&
        (() => {
          ...
          const sparkW = 120,
            sparkH = 24;
          const pts = ch
            .map((c, i) => `${(i / (ch.length - 1)) * sparkW},${sparkH - (c.maxConc / maxC) * sparkH}`)
            .join(" ");
          return (
...
export function MutagenesisView({ result, enzyme }: { result: MutagenesisResult; enzyme: EnzymeStructure }) {
  ...
  const effectColor = (e: string) => (e === "beneficial" ? THEME.MINT : e === "neutral" ? THEME.RISK_LOW : THEME.CORAL);
  const effectSymbol = (e: string) => (e === "beneficial" ? "+++" : e === "neutral" ? "·" : "−");
  ...
  return (
    <div style={{ padding: "10px 12px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <svg width={barW} height={12} style={{ overflow: "visible" }}>
            ...
            {enzyme.catalyticResidues.map((r) => (
              <rect
                key={`c-${r.position}`}
                x={(r.position / seqLen) * barW - 1}
                y={1}
                width={2}
                height={8}
                rx={1}
                fill="rgba(250,128,114,0.6)"
              />
            ))}
            {result.sites.map((s) => (
              <rect
                key={`m-${s.position}`}
                x={(s.position / seqLen) * barW - 1.5}
                y={0}
                width={3}
                height={10}
                rx={1}
                fill="rgba(147,203,82,0.5)"
              />
            ))}
          </svg>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-sm)", color: THEME.MINT, ...tn }}>
            {result.topCombination.predictedImprovement != null
        ...
        <tbody>
          {result.sites.map((s) => (
            <tr key={s.position} style={{ background: "rgba(255,255,255,0.015)" }}>
              ...
              <td style={{ ...dataCell, textAlign: "left", fontSize: "var(--nb-fs-xs)" }}>
                {s.suggestedMutants.join(",")}
              </td>
      ...
      <p
        style={{ margin: "6px 0 0", fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xxs)", color: LABEL, opacity: 0.7 }}
      >
        * ΔKcat/ΔKm predictions not available — quantitative mutagenesis effects require FoldX, Rosetta, or molecular
        dynamics.
      </p>
    </div>
  );
