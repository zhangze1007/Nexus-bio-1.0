  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      >
        ...
        <button
          onClick={handleInverseFolding}
          disabled={invFoldLoading}
      ...
      {invFoldResult && (
        ...
        >
          {invFoldResult.designNotes.map((n, i) => (
            <div key={i}>• {n}</div>
          ))}
        </div>
      )}
                ...
                <tr key={i} style={{ borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
                  ...
                  <td
                    style={{
                      ...
                      textAlign: "right",
                      color:
                        seq.score > 0.7 ? THEME.SUCCESS_HIGH : seq.score > 0.5 ? THEME.SUCCESS_MEDIUM : THEME.RISK_LOW,
                    }}
                  >
                    {seq.score.toFixed(3)}
                  </td>
                  <td style={{ padding: "6px 8px", textAlign: "right", color: VALUE }}>
                    {seq.metrics.packingQuality.toFixed(2)}
                  </td>
                  <td style={{ padding: "6px 8px", textAlign: "right", color: VALUE }}>
                    {seq.metrics.secondaryStructureMatch.toFixed(2)}
                  </td>
                  <td style={{ padding: "6px 8px", textAlign: "right", color: VALUE }}>
                    {seq.metrics.hydrophobicCoreIntegrity.toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            {invFoldResult.structuralMotifs.map((motif, i) => (
              <span
                ...
                style={{
                  ...
                  fontSize: THEME.FS_XS,
                  color:
                    motif.type === "helix" ? THEME.SKY : motif.type === "sheet" ? THEME.MINT : "rgba(255,255,255,0.5)",
                }}
              >
