...
export default React.memo(function CatDesSidebar({
  ...
  return (
    <div style={{
      ...
      height: '100%', overflowY: 'auto', padding: '10px 12px',
    }}>
      {/* ── 1. Enzyme Header ── */}
          ...
          {[
            ...
            <div key={ax.label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <div style={{ flex: 1, height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
                <div style={{ width: `${Math.min(100, ax.value * 100)}%`, height: '100%', borderRadius: 2, background: ax.color, opacity: 0.8 }} />
              </div>
              <span style={{ fontFamily: THEME.MONO, fontSize: '10px', color: VALUE, minWidth: 24, textAlign: 'right', ...tn }}>{ax.value.toFixed(2)}</span>
            </div>
          ))}
        </div>
        ...
        {dockingResult && (
          <div style={{ marginTop: 8, padding: '6px 8px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: `1px solid ${BORDER}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              ...
              <span style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: VALUE, ...tn }}>{dockingResult.dockingScore.toFixed(3)} kcal/mol</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ fontFamily: THEME.SANS, fontSize: 'var(--nb-fs-xs)', color: LABEL }}>Contacts</span>
              <span style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: VALUE, ...tn }}>{dockingResult.contactsFound}</span>
            </div>
      ...
      {selectedResidue != null && selectedCatResidue ? (
        <Section title="Selected Residue">
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            ...
            <span style={{
              fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: THEME.MINT,
              background: 'rgba(191,220,205,0.12)', padding: '2px 5px', borderRadius: 4,
            }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px 8px' }}>
            ...
            <div>
              ...
              <p style={{
                fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', margin: 0, ...tn,
                color: Math.abs(selectedCatResidue.pKaShift) > 0.5 ? THEME.CORAL : VALUE,
              }}>
                {selectedCatResidue.pKaShift > 0 ? '+' : ''}{selectedCatResidue.pKaShift.toFixed(2)}
              </p>
            </div>
          </div>
        </Section>
      ...
      <Section title="Mutation Predictor">
        <div style={{ marginBottom: 6 }}>
          <span style={{ fontFamily: THEME.SANS, fontSize: 'var(--nb-fs-xs)', color: LABEL, display: 'block', marginBottom: 3 }}>
            Mutate to
          </span>
          <select
            ...
            style={{
              width: '100%', fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-sm)', fontWeight: 600,
          ...
          >
            ...
            {'ACDEFGHIKLMNPQRSTVWY'.split('').filter(aa => selectedCatResidue ? aa !== selectedCatResidue.residue : true).map(aa => (
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 8px' }}>
              <div>
                ...
                <p style={{
                  fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-sm)', margin: 0, ...tn,
                  color: (mutationImpact.deltaG ?? 0) < 0 ? THEME.MINT : THEME.CORAL,
                }}>
                  {(mutationImpact.deltaG ?? 0) > 0 ? '+' : ''}{(mutationImpact.deltaG ?? 0).toFixed(2)} <span style={{ fontSize: 'var(--nb-fs-xxs)', color: LABEL }}>kcal/mol</span>
                </p>
              </div>
              <div>
                ...
                <p style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-sm)', color: VALUE, margin: 0, ...tn }}>
                  {(mutationImpact.newKd ?? 0).toFixed(2)} <span style={{ fontSize: 'var(--nb-fs-xxs)', color: LABEL }}>uM</span>
                </p>
              </div>
              <div>
                ...
                <p style={{
                  fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-sm)', margin: 0, ...tn,
                  color: (mutationImpact.confidence ?? 0) > 0.7 ? THEME.MINT : (mutationImpact.confidence ?? 0) > 0.5 ? THEME.RISK_LOW : THEME.CORAL,
                }}>
                  {((mutationImpact.confidence ?? 0) * 100).toFixed(0)}%
                </p>
              </div>
            </div>
            <p style={{ margin: '6px 0 0', fontFamily: THEME.SANS, fontSize: 'var(--nb-fs-xxs)', color: LABEL, opacity: 0.6, lineHeight: 1.4 }}>
