...
import { computeMutationImpact, fitQuality, kcatQuality, kdQuality, PHASE_COLORS } from "./catdes/catdesShared";
...
export default React.memo(function CatalystDesignerPage() {
  ...
  const { data: binding, error: simError } = useMemo(() => {
    try {
      return { data: predictBindingAffinity(enzyme), error: null as string | null };
    } catch (e) {
      return {
        data: predictBindingAffinity(ENZYME_STRUCTURES[selectedEnzyme]),
        error: e instanceof Error ? e.message : "Binding prediction failed",
      };
    }
  }, [enzyme]);
  const sequences = useMemo(
    () => designSequences(enzyme, recommendedSeed.designCount),
    [enzyme, recommendedSeed.designCount],
  );
  const drain = useMemo(
    () => estimateMetabolicDrain(activeEnzyme, recommendedSeed.requiredFlux),
    [activeEnzyme, recommendedSeed.requiredFlux],
  );
  ...
  return (
    ...
    >
      ...
      {fs.catdesError && (
        <div style={{ padding: "4px 16px 0" }}>
          <SimErrorBanner message={fs.catdesError} onRetry={() => fs.setCatdesError(null)} />
        </div>
      )}
      ...
      >
        ...
        <div
          style={{ display: "flex", flexDirection: "column", minHeight: 0, borderRight: `1px solid ${THEME.BORDER}` }}
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  setSelectedEnzyme(Number(e.target.value));
              ...
              >
                {ENZYME_STRUCTURES.map((enz, i) => (
                  <option key={enz.id} value={i}>
                    {enz.name} · EC {enz.ecNumber}
                  </option>
                ))}
              </select>
              ...
              {(["cartoon", "surface", "confidence"] as const).map((mode) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    fontSize: "var(--nb-fs-xxs)",
                    borderColor: renderMode === mode ? THEME.SKY : undefined,
                    background: renderMode === mode ? "rgba(175,195,214,0.15)" : undefined,
                    color: renderMode === mode ? THEME.SKY : undefined,
                  }}
                >
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                fontSize: "var(--nb-fs-xxs)",
                borderColor: spinEnabled ? THEME.MINT : undefined,
                background: spinEnabled ? "rgba(191,220,205,0.15)" : undefined,
                color: spinEnabled ? THEME.MINT : undefined,
              }}
            >
            ...
            <div
              style={{
                padding: "3px 8px",
                borderRadius: 4,
                border: `1px dashed ${fs.uploadedPdb ? THEME.MINT : INPUT_BORDER}`,
                background: fs.uploadedPdb ? "rgba(147,203,82,0.06)" : "transparent",
                cursor: "pointer",
                fontFamily: THEME.SANS,
                fontSize: "var(--nb-fs-xxs)",
                color: fs.uploadedPdb ? THEME.MINT : LABEL,
              }}
              onClick={() => {
                ...
                input.onchange = async (e) => {
                  ...
                  try {
                    ...
                    fs.setUploadedPdb(text);
                    fs.setUploadedPdbName(file.name);
                    fs.setCatdesError(null);
                  } catch (err) {
                    fs.setCatdesError(err instanceof Error ? err.message : "Failed to read PDB");
                  }
                };
                input.click();
              }}
            ...
            {fs.uploadedPdb && (
              <button
                onClick={() => {
                  fs.setUploadedPdb(null);
                  fs.setUploadedPdbName(null);
                }}
                style={{
                  fontFamily: THEME.SANS,
                  fontSize: "var(--nb-fs-xxs)",
            ...
            {/* ESMFold */}
            <button
              onClick={fs.handleESMFoldPredict}
              disabled={fs.esmfoldLoading || !activeEnzyme?.sequence}
              className="nb-tool-toggle"
              style={{
                padding: "3px 8px",
                borderRadius: 4,
                fontSize: "var(--nb-fs-xxs)",
                opacity: fs.esmfoldLoading ? 0.5 : 1,
              }}
            >
            ...
            {/* Docking */}
            <button
              onClick={fs.handleDocking}
              disabled={fs.dockingLoading || !enzyme.pdbId}
              className="nb-tool-toggle"
              style={{
                padding: "3px 8px",
                borderRadius: 4,
                fontSize: "var(--nb-fs-xxs)",
                opacity: fs.dockingLoading ? 0.5 : 1,
              }}
            >
          ...
          <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
            <CatalystViewer3D
              ...
              pdbText={fs.uploadedPdb || fs.esmfoldPdb}
              style={{ height: "100%" }}
            />
            <InlineMetricOverlay
              position="top-right"
              metrics={[
                ...
                ...(activeEnzyme.km > 0 && activeEnzyme.kcat > 0
                  ? [
                      {
                        label: "kcat/Km",
                        value: `${(activeEnzyme.kcat / activeEnzyme.km).toFixed(1)} mM-1s-1`,
                        accent: THEME.MINT,
                      },
                    ]
                  : []),
        ...
        <div style={{ minHeight: 0, overflow: "hidden" }}>
          <CatDesSidebar
            ...
            mutationImpact={
              mutationImpact
                ? { deltaG: mutationImpact.deltaG, newKd: mutationImpact.newKd, confidence: mutationImpact.confidence }
                : null
            }
          />
        </div>
      ...
      >
        <ExportButton
          ...
          data={{
            ...
            pareto,
            mutagenesis,
            docking: fs.dockingResult,
          }}
          filename="catalyst-design"
          format="json"
        />
        <ExportButton label="Export CSV" data={sequences.designs} filename="catalyst-sequences" format="csv" />
      </div>
