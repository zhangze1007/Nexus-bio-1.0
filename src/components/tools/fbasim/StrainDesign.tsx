      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          {pipelineResult?.bestDesign && (
            ...
            >
              ...
              >
                Growth: {pipelineResult.bestDesign.growthRate?.toFixed(4) ?? "N/A"} h⁻¹ | Product:{" "}
                {pipelineResult.bestDesign.productFlux?.toFixed(4) ?? "N/A"} | Burden:{" "}
                {(pipelineResult.bestDesign.growthFractionOfWT * 100).toFixed(1)}%
              </p>
        ...
        {strainDesignError && (
          <div style={{ padding: "0 16px 8px" }}>
            <SimErrorBanner message={strainDesignError} onRetry={() => setStrainDesignError(null)} />
          </div>
        )}
        ...
        <ScientificFigureFrame
          ...
          legend={[
            ...
            {
              label: "Model",
              value: loadedReactions ? `${loadedReactions.length} rxns` : "iJO1366 subset",
              accent: THEME.SKY,
            },
          ]}
        >
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
            <button
              ...
              style={{
                ...
                borderRadius: "var(--nb-radius-sm)",
                background:
                  strainDesignLoading || !loadedReactions ? "rgba(255,255,255,0.04)" : "rgba(191,220,205,0.14)",
                borderColor:
                  strainDesignLoading || !loadedReactions ? "rgba(255,255,255,0.08)" : "rgba(191,220,205,0.3)",
            ...
            <div style={{ overflow: "auto" }}>
              {fseofResult.wildType && (
                ...
                >
                  ...
                  >
                    Growth: {fseofResult.wildType.growthRate?.toFixed(4) ?? "N/A"} h⁻¹ | Product Flux:{" "}
                    {fseofResult.wildType.productFlux?.toFixed(4) ?? "N/A"} mmol/gDW/h
                  </p>
                </div>
              )}
                ...
                <tbody>
                  {(fseofResult.targets ?? []).map((t, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      <td style={{ padding: "5px 8px", textAlign: "right", color: "rgba(250,246,240,0.6)" }}>
                        {t.fluxAtMin.toFixed(3)}
                      </td>
                      <td style={{ padding: "5px 8px", textAlign: "right", color: "rgba(191,220,205,0.85)" }}>
                        {t.fluxAtMax.toFixed(3)}
                      </td>
                          ...
                          style={{
                            ...
                            background:
                              t.monotonicity >= 0.8
                                ...
                                : t.monotonicity >= 0.5
                                  ? "rgba(231,199,169,0.15)"
                                  : "rgba(232,163,161,0.15)",
                            color:
                              t.monotonicity >= 0.8
                                ? "rgba(191,220,205,0.9)"
                                : t.monotonicity >= 0.5
                                  ? "rgba(231,199,169,0.9)"
                                  : "rgba(232,163,161,0.9)",
                          }}
                        >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
            <button
              ...
              style={{
                ...
                borderRadius: "var(--nb-radius-sm)",
                background:
                  strainDesignLoading || !loadedReactions ? "rgba(255,255,255,0.04)" : "rgba(232,163,161,0.14)",
                borderColor:
                  strainDesignLoading || !loadedReactions ? "rgba(255,255,255,0.08)" : "rgba(232,163,161,0.3)",
            ...
            >
              {strainDesignLoading
                ? "Running..."
                : !loadedReactions
                  ? "Load BiGG Model First"
                  : "Run OptKnock (Heuristic)"}
            </button>
          </div>
          ...
          {optknockResult ? (
            <div style={{ overflow: "auto" }}>
              {optknockResult.wildType && (
                ...
                >
                  ...
                  >
                    Growth: {optknockResult.wildType.growthRate?.toFixed(4) ?? "N/A"} h⁻¹ | Product Flux:{" "}
                    {optknockResult.wildType.productFlux?.toFixed(4) ?? "N/A"} mmol/gDW/h
                  </p>
                </div>
              )}
                ...
                <tbody>
                  {(optknockResult.strategies ?? []).map((s, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      <td style={{ padding: "5px 8px", textAlign: "right", color: "rgba(191,220,205,0.85)" }}>
                        {s.growthRate.toFixed(4)}
                      </td>
                      <td style={{ padding: "5px 8px", textAlign: "right", color: "rgba(231,199,169,0.85)" }}>
                        {s.productFlux.toFixed(4)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            >
              Click "Run OptKnock (Heuristic)" to find knockout strategies that couple growth to product formation.
