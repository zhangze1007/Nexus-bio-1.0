...
type SingleSpeciesFBAProps = Pick<FBASimState,
  | 'glucoseUptake' | 'setGlucoseUptake'
  | 'oxygenUptake' | 'setOxygenUptake'
  | 'objective' | 'setObjective'
  | 'knockouts' | 'setKnockouts' | 'toggleKO'
  | 'singleResult' | 'singleError' | 'setSingleError' | 'singleLoading'
  | 'chartRef'
  | 'biggModels' | 'biggResult' | 'selectedModel' | 'setSelectedModel'
  | 'biggLoading' | 'loadedReactions' | 'loadedObjectiveId' | 'modelLoading' | 'handleLoadModel'
  | 'seedOverwriteNotice' | 'setSeedOverwriteNotice' | 'lastAppliedSeedRef'
  | 'recommendedSeed'
  | 'figureMeta'
>;
...
export function FluxMapTab(props: SingleSpeciesFBAProps) {
  const {
    ...
    objective, setObjective, knockouts, toggleKO,
    singleResult, singleError, setSingleError, singleLoading,
    chartRef,
    biggModels, biggResult, selectedModel, setSelectedModel,
    biggLoading, loadedReactions, loadedObjectiveId, modelLoading, handleLoadModel,
    seedOverwriteNotice, setSeedOverwriteNotice, lastAppliedSeedRef,
    recommendedSeed, figureMeta,
  } = props;
...
export function KnockoutTab(props: SingleSpeciesFBAProps) {
  const {
    knockouts, toggleKO, setKnockouts,
    singleResult, singleError, setSingleError,
    chartRef,
  } = props;
