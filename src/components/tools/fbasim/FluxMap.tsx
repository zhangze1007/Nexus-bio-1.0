...
import type { FBAOutput, FLUX_EDGES, METABOLIC_NODES } from "../../../data/mockFBA";
import { THEME } from "../../../theme";
import { PAPER_THEME, SCI_PALETTE, SCI_PASTEL_MUTED } from "../../charts/chartTheme";
...
export function runForceLayout(
  ...
  tcaNodes.forEach((n, i) => {
    pos[n.id] = {
      x: width * 0.55 + (i % 2) * 50,
      y: PAD + 80 + i * ((height - PAD * 2 - 80) / Math.max(tcaNodes.length, 1)),
    };
  });
  ...
  for (let iter = 0; iter < 180; iter++) {
    ...
    edges.forEach((e) => {
      ...
      const dx = pos[e.to].x - pos[e.from].x,
        dy = pos[e.to].y - pos[e.from].y;
      const d = Math.max(Math.sqrt(dx * dx + dy * dy), 0.01);
      const att = (d * d) / k;
      disp[e.from].dx += (dx / d) * att;
      disp[e.from].dy += (dy / d) * att;
      disp[e.to].dx -= (dx / d) * att;
      disp[e.to].dy -= (dy / d) * att;
    });
    nodeIds.forEach((id) => {
...
export function FluxMap({
  ...
  const positions = useMemo(() => runForceLayout(nodes, edges, W, viewH), [nodes, edges, viewH]);
  ...
  return (
    ...
    >
      ...
      <text x="28" y="22" fontFamily={THEME.MONO} fontSize="10" fill={SUBSYSTEM_COLORS.Glycolysis} opacity={0.75}>
        ● GLYCOLYSIS
      </text>
      <text x="200" y="22" fontFamily={THEME.MONO} fontSize="10" fill={SUBSYSTEM_COLORS.TCA} opacity={0.75}>
        ● TCA CYCLE
      </text>
        ...
        return (
          <g key={edge.reactionId}>
            ...
            <text
              x={mx}
              y={my + 4}
              fill={isKO ? "#C0392B" : PAPER_THEME.tickColor}
      ...
      <text x={W - 96} y={40} fontFamily={THEME.MONO} fontSize="10" fill={PAPER_THEME.legendColor}>
        μ BIOMASS
      </text>
