      ...
      {iterations.map((it, i) => {
        ...
        return (
          <g key={it.id}>
            ...
            <text x={34} y={y + 20} textAnchor="middle" fontFamily={MONO} fontSize="8"
              fill={phaseColor}>
    ...
    <IDEShell moduleId="dbtlflow">
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: '#0a0c10' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
          {/* Input panel */}
          <div style={{ width: '260px', flexShrink: 0, overflowY: 'auto', padding: '16px', borderRight: '1px solid rgba(255,255,255,0.06)', background: '#0a0c10' }}>
            ...
            <div style={{ marginBottom: '10px' }}>
              <label style={{ fontFamily: SANS, fontSize: '11px', color: 'rgba(255,255,255,0.45)', display: 'block', marginBottom: '4px' }}>
                Hypothesis
              </label>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
              {[true, false].map(p => (
                <button key={String(p)} onClick={() => setPassed(p)} style={{
                  flex: 1, padding: '6px',
                  background: passed === p ? (p ? 'rgba(120,220,180,0.12)' : 'rgba(255,80,80,0.12)') : 'transparent',
                  border: `1px solid ${passed === p ? (p ? 'rgba(120,220,180,0.4)' : 'rgba(255,80,80,0.4)') : 'rgba(255,255,255,0.08)'}`,
                  borderRadius: '3px',
                  color: passed === p ? (p ? 'rgba(120,220,180,0.9)' : 'rgba(255,100,80,0.9)') : 'rgba(255,255,255,0.3)',
            ...
            <div style={{ marginTop: '16px', padding: '10px', background: 'rgba(255,255,255,0.02)', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.05)' }}>
              <p style={{ fontFamily: MONO, fontSize: '9px', color: 'rgba(255,255,255,0.25)', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Best Result</p>
              <p style={{ fontFamily: MONO, fontSize: '14px', color: 'rgba(120,220,180,0.85)', margin: '0 0 4px' }}>
                {bestIteration?.result} {bestIteration?.unit}
              </p>
              <p style={{ fontFamily: SANS, fontSize: '10px', color: 'rgba(255,255,255,0.3)', margin: 0, lineHeight: 1.4 }}>
