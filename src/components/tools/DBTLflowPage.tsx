    ...
    >
      {s.dbtlError && (
        <div style={{ padding: "0 0 8px" }}>
          <SimErrorBanner message={s.dbtlError} onRetry={() => s.setDbtlError(null)} />
        </div>
      )}
      ...
      <ToolTabPanel activeId={s.activeTab} tabId="cycle">
          ...
          <ScientificHero
            ...
            signals={[
              ...
              },
              {
                ...
                detail: s.hasCommittedFeedback
                  ? "Approved typed deltas are still required before seed builders can apply changes."
                  : "Learning is still visible, but not yet cleared for upstream reseeding.",
        ...
        <div style={{ padding: "0 16px 6px" }}>
          ...
          >
            ...
            <div
              style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.paperValue, lineHeight: 1.45 }}
            >
              {s.feedbackGateLabel}
            </div>
            <div
              style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.paperLabel, lineHeight: 1.4 }}
            >
              committed pass rate {s.committedPassRate}% · committed improvement {s.committedImprovementRate} · latest
              committed phase {s.latestCommittedIteration?.phase ?? "Design"}
            </div>
          </div>
        </div>
            ...
            items={[
              {
                ...
                detail:
                  "Hypothesis, result, and pass/fail stay editable on the left so the next cycle enters the record with explicit context instead of becoming anonymous row data.",
              ...
              },
              {
                ...
                detail:
                  "Progress ring, phase legend, and iteration timeline are merged into one figure frame so experimental history reads like a ledger panel, not a utility dashboard.",
                accent: THEME.sky,
              ...
              },
              {
                ...
                detail:
                  "Automation, provenance, and feedback remain attached on the right so only governed learn output can return upstream.",
                accent: THEME.mint,
      ...
      <ToolTabPanel activeId={s.activeTab} tabId="closedloop">
        <ClosedLoopDBTLPanel iterations={s.iterations} />
      </ToolTabPanel>
      ...
      >
        <DataSourceBadge
          source={s.iterations.length > INITIAL_ITERATIONS.length ? "live" : "engine"}
          label={
            s.iterations.length > INITIAL_ITERATIONS.length ? "User Data + GP Engine" : "Demo Data + GP/Bayesian Engine"
          }
        />
