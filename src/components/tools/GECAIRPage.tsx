...
import { DynamicsTabPanel, ODEMiniDynamics } from "./gecair/CircuitDynamics";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 16 }}>
      ...
      >
        ...
        <textarea
          value={truthTableRows}
          onChange={(e) => setTruthTableRows(e.target.value)}
          rows={4}
          cols={20}
          style={{
            padding: "4px 8px",
        ...
        <>
          ...
          >
            {[
              ...
              {
                label: "Burden",
                value: (result.burden * 100).toFixed(0) + "%",
                color: result.burden > 0.3 ? "rgba(250,128,114,0.7)" : "rgba(147,203,82,0.7)",
              },
            ].map((m, i) => (
    ...
    >
      ...
      <ToolTabPanel activeId={activeTab} tabId="circuit">
          ...
          <ScientificHero
            ...
            aside={
              <>
                ...
                >
                  Catalyst burden and controller stability are already being used here to bias the circuit topology
                  instead of leaving logic selection arbitrary.
                </div>
              </>
            }
            signals={[
              ...
              },
              {
                ...
                detail:
                  noiseScore > 0.05
                    ? "Circuit is sensitive to small input perturbations and may need insulation."
                    : "Noise remains in a manageable range for this design.",
              ...
              },
              {
                ...
                detail:
                  "These inputs are seeded from the current control and catalyst state, not manually invented defaults.",
                tone: "neutral",
              },
              {
            ...
            items={[
              {
                ...
                detail:
                  "The controller and catalyst state seed the gate inputs so circuit design starts from system pressure instead of abstract binary examples.",
                accent: THEME.sky,
              ...
              },
              {
                ...
                detail:
                  "Promoter, RBS, CDS, terminator, and phase-space response are grouped into one publication-style figure rather than split across dashboard cards.",
                accent: THEME.lilac,
                note: `${gateType} gate`,
              },
              {
                ...
                detail:
                  "Noise sensitivity and output level remain visible next to the figure so buildability and control quality stay attached to the same decision.",
                accent: THEME.mint,
        ...
        <div className="nb-tool-panels" style={{ flex: 1 }}>
          ...
          <div
            ...
            style={{ width: "240px", borderRight: `1px solid ${THEME.paperBorder}`, background: THEME.sepiaPanelMuted }}
          >
            <p
              style={{
            ...
            {(["NOT", "AND", "OR", "NAND"] as GateType[]).map((gate) => (
              <button
                aria-label={`Select ${gate} gate type`}
                key={gate}
                onClick={() => setGateType(gate)}
                className={`nb-tool-toggle ${gateType === gate ? "nb-tool-toggle--active" : ""}`}
              >
                {gate} Gate
              </button>
            ))}
                      ...
                      {[row.A, row.B, out].map((v, j) => (
                        <td
                          ...
                          style={{
                            ...
                            textAlign: "center",
                            padding: "4px",
                            color: v ? THEME.mint : THEME.paperLabel,
                          }}
                        >
                          {v ? "1" : "0"}
                        </td>
          ...
          <div
            ...
            style={{ flex: 1, background: THEME.sepiaPanelMuted, padding: "12px", minWidth: 0 }}
          >
            <ScientificFigureFrame
              ...
              footer={
                <div style={{ display: "grid", gap: "6px" }}>
                  ...
                  >
                    The page now treats the circuit as a scientific control object: architecture, phase space, transfer
                    response, and node state are presented as one figure so logic choice can be defended from first
                    principles.
                  </div>
                  <div style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.paperLabel }}>
                    recommended gate {recommendedGate} · node outputs {(outA * 100).toFixed(0)} /{" "}
                    {(outB * 100).toFixed(0)} · noise {noiseScore.toFixed(4)}
                  </div>
                </div>
              }
              minHeight="100%"
          ...
          <div
            ...
            style={{ width: "240px", borderLeft: `1px solid ${THEME.paperBorder}`, background: THEME.sepiaPanelMuted }}
          >
            <p
              style={{
                fontFamily: THEME.SANS,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <MetricCard
                label="Noise Sensitivity"
                value={noiseScore.toFixed(4)}
                warning={noiseScore > 0.05 ? "High noise sensitivity — consider insulator parts" : undefined}
              />
              <MetricCard
                label="Circuit Complexity"
                value={CIRCUIT_NODES.reduce((a, n) => a + n.parts.length, 0)}
                unit="parts"
              />
            </div>
            ...
            >
              ...
              {circuitType === "toggle_switch" && (
                <div style={{ display: "flex", gap: "6px", marginTop: "4px" }}>
                  <span
                    style={{ fontFamily: THEME.MONO, fontSize: "11px", color: THEME.paperLabel, alignSelf: "center" }}
                  >
                    Perturbation:
                  </span>
                  {(["A", "B"] as const).map((p) => (
            ...
            >
              ...
              {pipelineError && (
                <p
                  style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.CORAL, margin: "6px 0 0" }}
                >
                  {pipelineError}
                </p>
              )}
              {pipelineResult && (
                ...
                >
                  <div style={{ fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)", color: THEME.paperValue }}>
                    Gate: {pipelineResult.recommendedGate} | Out: {(pipelineResult.outputLevel * 100).toFixed(1)}% |
                    Noise: {pipelineResult.noiseScore.toFixed(4)}
                  </div>
                  <div
                    style={{
                      fontFamily: THEME.MONO,
      ...
      <ToolTabPanel activeId={activeTab} tabId="dynamics">
        <DynamicsTabPanel
          ...
          setStochasticMode={setStochasticMode}
          ensembleRuns={ensembleRuns}
          setEnsembleRuns={setEnsembleRuns}
          togglePerturbation={togglePerturbation}
          setTogglePerturbation={setTogglePerturbation}
          stochasticEnsemble={stochasticEnsemble}
        />
      </ToolTabPanel>
      {/* ═══════ TRUTH TABLE TAB ═══════ */}
                    ...
                    {[row.A, row.B, out].map((v, j) => (
                      <td
                        ...
                        style={{
                          ...
                          textAlign: "center",
                          padding: "8px 12px",
                          color: v ? THEME.MINT : THEME.LABEL,
      ...
      >
        <DataSourceBadge
          source={catalystPayload || dynconPayload ? "live" : "mock"}
          label={catalystPayload || dynconPayload ? "Upstream Data" : "Default Inputs"}
        />
