...
import { type IntentRoute, routeIntent } from "../../services/axonIntentRouter";
import {
  computeVerificationSummary,
  mergeVerificationResults,
  verifyCitationsBatch,
} from "../../services/citationVerifier";
import { formatSolverResult, routeQuery, setCachedResponse } from "../../services/cognitiveRouter";
import { computeConfidenceFromResult } from "../../services/confidenceEngine";
import { useUIStore } from "../../store/uiStore";
import { useWorkbenchStore } from "../../store/workbenchStore";
import { THEME } from "../../theme";
import type { BottleneckEnzyme, CitationNode, GeneratedPathway, NEXAIResult } from "../../types";
...
import ResultPanel, { type ParseErrorInfo } from "./nexai/ResultPanel";
  ...
  if (axon?.question) {
    ...
    if (bottlenecks.length > 0) {
      const bList = bottlenecks
        .map(
          (b: BottleneckEnzyme) =>
            `${b.enzyme} (${b.efficiency_percent}% efficiency, ${b.yield_loss_percent}% yield loss)`,
        )
        .join("; ");
...
export default React.memo(function NEXAIPage() {
  ...
  useEffect(() => {
    if (result?.citations && result.citations.length > 0 && !verified) {
      ...
      verifyCitationsBatch(toVerify, controller.signal)
        .then((batchResults) => {
  ...
  const copilotContext = useMemo(
    () =>
      buildWorkbenchCopilotContext({
        targetProduct: null,
        project: project ? { title: project.title, targetProduct: project.targetProduct } : null,
  ...
  async function runQuery() {
    ...
    appendConsole({
      level: "info",
      module: "nexai",
      message: `Query: "${activeQuery.slice(0, 60)}${activeQuery.length > 60 ? "…" : ""}"`,
    });

    // ── Cognitive Kernel: tier routing ──
    const routing = routeQuery(
      activeQuery,
      typeof copilotContext === "string" ? copilotContext : JSON.stringify(copilotContext).substring(0, 200),
    );
    ...
    try {
      ...
      // ── Tier 2: Solver + LLM explain ──
        ...
        appendConsole({
          level: "success",
          module: "nexai",
          message: `Solver + LLM: ${routing.solver} + ${resolvedProvider}`,
        });
      }
      ...
      if (pathway) {
        ...
        appendConsole({
          level: "success",
          module: "nexai",
          message: `Axon: ${pathway.nodes.length} nodes · ${bottlenecks} bottleneck(s) · ${resolvedProvider}`,
        });
      } else {
        appendConsole({
          level: "success",
          module: "nexai",
          message: `Response: ${confidenceResult.badge} confidence (${confidenceResult.level}) · ${routing.tier}`,
        });
      }
      ...
      try {
        ...
        if (ssRes.ok) {
          ...
          const maxCitations = Math.max(
            1,
            ...((ssData.data ?? []) as Record<string, unknown>[]).map((p) => (p.citationCount as number) ?? 0),
          );
          ...
          if (ssCitations.length > 0) {
            setResult((prev) =>
              prev
                ? {
                    ...
                    citations: [
                      ...ssCitations,
                      ...prev.citations.filter((c) => !ssCitations.find((s) => s.id === c.id)),
                    ].slice(0, 10),
                  }
                : prev,
            );
            appendConsole({
              level: "info",
              module: "nexai",
              message: `Semantic Scholar: ${ssCitations.length} real citation(s) loaded`,
            });
          }
        } else {
          setSsSource("mock");
        ...
        appendConsole({
          level: "warn",
          module: "nexai",
          message: `Semantic Scholar enrichment unavailable: ${ssErr instanceof Error ? ssErr.message.slice(0, 80) : "network error"}`,
        });
      }
      ...
      appendConsole({
        level: "error",
        module: "nexai",
        message: `${providerLabel} API unavailable — ${errMsg.slice(0, 120)}`,
      });
      setApiError(
        `${providerLabel} API unavailable — ${errMsg}. Please verify your API key is configured and try again.`,
      );
      setResult(null);
      setResultMode("idle");
      setSurfaceView("answer");
  ...
  async function verifyCitations() {
    ...
    appendConsole({
      level: "info",
      module: "nexai",
      message: `Verifying ${result.citations.length} citation(s) against PubMed...`,
    });
    ...
    try {
      ...
      appendConsole({
        level: "warn",
        module: "nexai",
        message: `Citation verification failed: ${e instanceof Error ? e.message : String(e)}`,
      });
    }
    setVerifying(false);
  }
  ...
  const malformedParse = parseError && (parseError.code === "INVALID_SYNTAX" || parseError.code === "EMPTY");
  const workflowRisk =
    workflowControl.status === "blocked"
          ...
          <ScientificHero
            ...
            aside={
              <>
                ...
                <div
                  style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.VALUE, fontWeight: 700 }}
                >
                  {analyzeArtifact?.targetProduct ?? project?.targetProduct ?? project?.title ?? "Scientific workbench"}
                </div>
                <div
                  style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.LABEL, lineHeight: 1.55 }}
                >
                  {contextPrompt ||
                    "Ask Axon to synthesise evidence, explain a bottleneck, or route the next scientific action."}
                </div>
              </>
            }
            signals={[
            ...
            items={[
              {
                ...
                detail:
                  "The active target product, evidence graph, and queued next-step recommendations seed the research prompt so Axon starts from the workbench state.",
              ...
              },
              {
                ...
                detail:
                  "The written synthesis is the default reading plane. Evidence graph and raw structured output remain one click away for power users.",
                accent: THEME.SKY,
              ...
              },
              {
                ...
                detail:
                  "When the model fails to produce the structured envelope we asked for, the failure is surfaced explicitly rather than coerced into a plausible brief.",
                accent: THEME.MINT,
                note: malformedParse
                  ? (parseError?.code ?? "unknown")
                  : parseError?.code === "NO_OBJECT"
                    ? "prose"
                    : "ok",
              },
            ]}
        ...
        >
          ...
          {messages.length > 0 ? (
            ...
            >
              ...
              {loading && <ChatMessage role="assistant" content="" isLoading timestamp={Date.now()} />}
              ...
              steps={[
                {
                  id: "classify",
                  label: "Classify",
                  status: workflowStep === "classify" ? "active" : loading || result ? "done" : "pending",
                },
                {
                  id: "route",
                  label: "Route",
                  status:
                    workflowStep === "route"
                      ? "active"
                      : ["execute", "synthesize"].includes(workflowStep) || result
                        ? "done"
                        : "pending",
                },
                {
                  ...
                  status:
                    workflowStep === "execute"
                      ? "active"
                      : workflowStep === "synthesize" || result
                        ? "done"
                        : "pending",
                },
                {
                  id: "synthesize",
                  label: "Synthesize",
                  status: workflowStep === "synthesize" ? "active" : result ? "done" : "pending",
                },
              ]}
              activeIndex={
                workflowStep === "classify" ? 0 : workflowStep === "route" ? 1 : workflowStep === "execute" ? 2 : 3
              }
            />
          )}
              ...
              metrics={[
                {
                  ...
                  accent: result.confidence > 0.7 ? THEME.MINT : result.confidence > 0.4 ? THEME.APRICOT : THEME.CORAL,
                },
        ...
        <ModuleCard area="stats" title="Query Stats">
          <div style={{ display: "flex", flexDirection: "column", gap: "8px", flex: 1 }}>
                  ...
                  (() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          style={{ display: "flex", gap: "8px", fontFamily: THEME.MONO, fontSize: "var(--nb-fs-xs)" }}
                        >
            ...
            >
              ...
              <div
                style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-sm)", color: THEME.VALUE, lineHeight: 1.55 }}
              >
                {workflowControl.explanation}
              </div>
              <div
              ...
              >
                ...
                <span>
                  confidence · {workflowControl.confidence === null ? "unknown" : workflowControl.confidence.toFixed(2)}
                </span>
                <span>
                  uncertainty ·{" "}
                  {workflowControl.uncertainty === null ? "unknown" : workflowControl.uncertainty.toFixed(2)}
                </span>
                <span>
                  missing evidence · {workflowControl.missingEvidence.have}/
                  {workflowControl.missingEvidence.minRequired}
                  {workflowControl.missingEvidence.kinds.length
                    ? ` ${workflowControl.missingEvidence.kinds.join(", ")}`
                    : ""}
                </span>
