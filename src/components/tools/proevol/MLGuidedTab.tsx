...
import { PROEVOL_THEME, StatusPill, tableHeaderStyle, tableCellStyle } from './shared';
import { kicker, CompactMetric, BOImprovementChart } from './sharedComponents';
import type { ProEvolState } from './useProEvolState';

export default function MLGuidedTab({ state }: { state: ProEvolState }) {
  const {
    mlMode, setMlMode, gpPredictions, suggestedVariantId, setSelectedVariantId,
    gpError, mlVariants, gpTableRows, runGPAnalysis,
    boResult, boRunning, boError, boAcqType, setBoAcqType,
    boRounds, setBoRounds, boBatchSize, setBoBatchSize, runBOSimulation,
  } = state;
  return (
    <div style={{ padding: '16px', display: 'grid', gap: '12px' }}>
      ...
      {mlMode && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            ] as const).map(({ label, value, set, min, max, fallback }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: PROEVOL_THEME.label }}>{label}</span>
                <input type="number" min={min} max={max} value={value}
                  onChange={(e) => set(Math.max(min, Math.min(max, Number(e.target.value) || fallback)))}
                  style={{ width: 48, padding: '3px 6px', borderRadius: 'var(--nb-radius-sm)', background: PROEVOL_THEME.inset, border: `1px solid ${PROEVOL_THEME.border}`, fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: PROEVOL_THEME.value, textAlign: 'center' }}
                />
              </div>
            ))}
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              <span style={{ fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: PROEVOL_THEME.label }}>Acquisition</span>
              <select value={boAcqType} onChange={(e) => setBoAcqType(e.target.value as 'EI' | 'UCB' | 'EHVI')}
                style={{ padding: '3px 6px', borderRadius: 'var(--nb-radius-sm)', background: PROEVOL_THEME.inset, border: `1px solid ${PROEVOL_THEME.border}`, fontFamily: THEME.MONO, fontSize: 'var(--nb-fs-xs)', color: PROEVOL_THEME.value }}>
      ...
      {/* Empty state */}
      {mlMode && gpPredictions.length === 0 && (
        <div style={{ padding: '20px', borderRadius: 'var(--nb-radius-md)', border: `1px dashed ${PROEVOL_THEME.border}`, background: PROEVOL_THEME.surface, textAlign: 'center' }}>
      ...
      {!mlMode && (
        <div style={{ padding: '20px', borderRadius: 'var(--nb-radius-md)', border: `1px dashed ${PROEVOL_THEME.border}`, background: PROEVOL_THEME.surface, textAlign: 'center' }}>
          <div style={{ fontFamily: THEME.SANS, fontSize: 'var(--nb-fs-sm)', color: PROEVOL_THEME.muted, lineHeight: 1.6 }}>
