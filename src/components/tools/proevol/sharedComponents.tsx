...
export function deriveMutations(variantId: string): { mutationString: string; mutationBurden: number; mutations: Array<{ position: number; from: string; to: string }> } {
  if (variantId === 'WT' || variantId.toLowerCase().startsWith('wt')) {
...
export function CompactMetric({ label, value, delta, accent }: { label: string; value: string; delta: string; accent: string }) {
  return (
...
export function ChartShell({ title, children, footnote }: { title: string; children: React.ReactNode; footnote?: string }) {
  return (
    <div style={{
