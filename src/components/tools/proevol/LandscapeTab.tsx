...
import { formatSigned, PROEVOL_THEME, StatusPill } from "./shared";
import { ChartShell, CompactMetric, InfoField, kicker, SectionKicker } from "./sharedComponents";
  ...
  return (
    <div style={{ display: "grid", gap: "10px", padding: "10px 12px 14px" }}>
        ...
        {!csvArtifact ? (
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            ...
            >
              ...
              <code style={{ fontFamily: THEME.MONO, color: PROEVOL_THEME.sky, fontSize: "var(--nb-fs-xs)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            ...
            <span style={{ fontFamily: THEME.SANS, fontSize: "var(--nb-fs-xs)", color: PROEVOL_THEME.muted }}>
              {csvArtifact.provenance.replicateCount} replicates ·{" "}
              {csvArtifact.provenance.bandSemantic === "measurement" ? "95% CI bands" : "model spread"}
            </span>
            <button
              type="button"
              onClick={clearCSV}
            ...
            >
              Replace
              <input type="file" accept=".csv,text/csv" onChange={handleFileInputChange} style={{ display: "none" }} />
            </label>
          </div>
        )}
        {uploadError ? (
      ...
      >
        <CompactMetric
          label="Lead score"
          value={lead.score.composite.toFixed(1)}
          delta={formatSigned(lead.score.deltaFromWildType, 1)}
          accent={PROEVOL_THEME.mint}
        />
        <CompactMetric
          label="Activity"
          value={lead.predictedActivity.toFixed(1)}
          delta={formatSigned(lead.predictedActivity - wt.predictedActivity, 1)}
          accent={PROEVOL_THEME.mint}
        />
        <CompactMetric
          label="Stability"
          value={lead.predictedStability.toFixed(1)}
          delta={formatSigned(lead.predictedStability - wt.predictedStability, 1)}
          accent={PROEVOL_THEME.sky}
        />
        <CompactMetric
          label="Expression"
          value={lead.predictedExpression.toFixed(1)}
          delta={formatSigned(lead.predictedExpression - wt.predictedExpression, 1)}
          accent={PROEVOL_THEME.apricot}
        />
        <CompactMetric
          label="Specificity"
          value={lead.predictedSpecificity.toFixed(1)}
          delta={formatSigned(lead.predictedSpecificity - wt.predictedSpecificity, 1)}
          accent={PROEVOL_THEME.lilac}
        />
        <CompactMetric
          label="Confidence"
          value={`${lead.confidence.toFixed(0)}%`}
          delta={`R${lead.round}`}
          accent={PROEVOL_THEME.lilac}
        />
      </div>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <StatusPill tone={lead.riskFlags.length ? "warm" : "neutral"}>burden {lead.mutationBurden}</StatusPill>
      ...
      <div style={{ display: "grid", gap: "8px", gridTemplateColumns: "minmax(0, 2.4fr) minmax(200px, 0.8fr)" }}>
        <ChartShell
          ...
          footnote={`Frequencies use Laplace pseudocount (+1). Hover for ${bandSemantic === "modeled" ? "model spread" : "95% CI"} range.`}
        >
      ...
      <ChartShell
        ...
        footnote="Above dashed line = enriched vs WT. Bubble area = final frequency."
      >
        <EnrichmentBurdenScatter
          entries={activeResearch.enrichment}
          highlightVariantId={selectedVariantId}
          onSelectVariant={setSelectedVariantId}
        />
      </ChartShell>
