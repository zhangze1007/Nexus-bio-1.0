...
function getFormulaText(type: InhibitionType): string {
  switch (type) {
    ...
    case "competitive":
      return "v = Vmax × [S] / (Km × (1 + [I]/Ki) + [S])";
    case "uncompetitive":
      return "v = Vmax × [S] / (Km + [S] × (1 + [I]/Kiu))";
    case "mixed":
      return "v = Vmax × [S] / (Km × (1 + [I]/Kic) + [S] × (1 + [I]/Kiu))";
    case "substrate":
      return "v = Vmax × [S] / (Km + [S] + [S]²/Kis)";
  }
}
...
function EstimationChart({
  ...
  const xMin = 0,
    xMax = Math.max(...allX) * 1.05 || 1;
  const yMin = 0,
    yMax = Math.max(...allY) * 1.1 || 1;
  ...
  return (
    <div>
      ...
      <svg width={W} height={H} style={{ overflow: "visible" }}>
        ...
        {data.map((d, i) => (
          <circle key={i} cx={px(d.s)} cy={py(d.v)} r={3.5} fill="none" stroke="#FA8072" strokeWidth={1.5} />
        ))}
      </svg>
  ...
  const sensitivities = useMemo(() => {
    ...
    if (inhibitionType === "competitive" || inhibitionType === "mixed") {
      ...
      entries.push({
        name: "Ki",
        baseValue: base,
        build: (d) => buildVelocityFn(inhibitionType, Vmax, Km, base + d, Kiu, Kis, I),
      });
    }
    if (inhibitionType === "uncompetitive" || inhibitionType === "mixed") {
      ...
      entries.push({
        name: "Kiu",
        baseValue: base,
        build: (d) => buildVelocityFn(inhibitionType, Vmax, Km, Ki, base + d, Kis, I),
      });
    }
    if (inhibitionType === "substrate") {
      ...
      entries.push({
        name: "Kis",
        baseValue: base,
        build: (d) => buildVelocityFn(inhibitionType, Vmax, Km, Ki, Kiu, base + d, I),
      });
    }
...
Be specific and scientific. No generic statements.`;
    ...
    try {
      ...
      setAi({
        text:
          text.trim() ||
          buildKineticFallbackInterpretation({
            nodeLabel,
            duration,
            finalProduct: finalP,
    ...
    setTimeout(() => {
      try {
        const initialGuess: number[] = (() => {
          switch (estModel) {
            case "competitive":
              return [Vmax, Km, Ki > 0 ? Ki : 0.5];
            case "uncompetitive":
              return [Vmax, Km, Kiu > 0 ? Kiu : 0.5];
            case "mixed":
              return [Vmax, Km, Ki > 0 ? Ki : 0.5, Kiu > 0 ? Kiu : 0.5];
          }
        })();
  ...
  const fittedCurve = useMemo(() => {
    ...
    const y = x.map((s) => {
      ...
      switch (estModel) {
        case "competitive":
          return competitiveInhibition(p[0], s, p[1], p[2], avgI);
        case "uncompetitive":
          return uncompetitiveInhibition(p[0], s, p[1], p[2], avgI);
        case "mixed":
          return mixedInhibition(p[0], s, p[1], p[2], p[3], avgI);
      }
    });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {inhibitionType !== "none" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
              {(inhibitionType === "competitive" || inhibitionType === "mixed") && (
                <InputField
                  label={inhibitionType === "mixed" ? "Kic" : "Ki"}
                  value={Ki}
                  unit="mM"
                  onChange={setKi}
                  min={0.001}
                  max={100}
                  step={0.01}
                  hint={inhibitionType === "mixed" ? "Competitive inhibition constant" : "Inhibition constant"}
                />
              )}
              {(inhibitionType === "uncompetitive" || inhibitionType === "mixed") && (
                <InputField
                  ...
                  max={100}
                  step={0.01}
                  hint="Uncompetitive inhibition constant"
                />
              )}
              {inhibitionType === "substrate" && (
                <InputField
                  ...
                  max={1000}
                  step={0.1}
                  hint="Substrate inhibition constant"
                />
              )}
              {inhibitionType !== "substrate" && (
                <InputField
      ...
      {activeTab === "simulation" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            {result && <ActionButton variant="secondary" size="md" icon={<RotateCcw size={13} />} onClick={reset} />}
          </div>
          ...
          >
            <LineChart
              data={mmCurve}
              color="#C8D8E8"
              label={`Michaelis-Menten curve (${inhibitionLabel(inhibitionType)} model)`}
              unit="mM"
            />
            <p
              style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              >
                <LineChart
                  ...
                  color="#E8DCC8"
                  label="Reaction velocity over time"
                  unit="min"
                />
              </div>
      ...
      {activeTab === "estimation" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <SelectField
            ...
            value={estModel}
            onChange={(v) => {
              setEstModel(v as InhibitionModel);
              setEstResult(null);
            }}
            options={[
          ...
          <div>
            ...
            {estData.length > 0 && (
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: estModel !== "competitive" ? "1fr 1fr 1fr 28px" : "1fr 1fr 28px",
                  gap: "6px",
                  marginBottom: "6px",
                  padding: "0 2px",
                }}
            ...
            <button
              ...
              style={{
                ...
                padding: "7px 12px",
                background: "rgba(200,216,232,0.05)",
                border: "1px dashed rgba(200,216,232,0.15)",
                borderRadius: "10px",
                color: "rgba(200,216,232,0.5)",
                fontSize: "10px",
                fontFamily: THEME.SANS,
          ...
          <ActionButton
            ...
            size="md"
            icon={
              estimating ? <Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} /> : <Play size={13} />
            }
          ...
          {estResult && (
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "6px" }}>
                {[
                  ...
                  >
                    ...
                    <p
                      style={{
                        color: m.l === "Converged" && !estResult.converged ? "#FA8072" : "rgba(255,255,255,0.7)",
                        fontSize: "11px",
                        fontFamily: THEME.SANS,
                        fontFeatureSettings: "'tnum' 1",
                        margin: 0,
              ...
              {/* Apply button */}
              <ActionButton variant="secondary" size="md" onClick={applyEstimation}>
      ...
      {activeTab === "sensitivity" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          >
            ...
            <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "10px", margin: "3px 0 0" }}>
              S = (∂v/∂p) × (p/v) at [S] = {S0.toFixed(2)} mM. A coefficient of 1.0 means a 1% change in the parameter
              causes a 1% change in velocity.
            </p>
          </div>
          ...
          >
            ...
            >
              {velocityFn(S0).toFixed(4)} μmol/min/mg
            </span>
          </div>
            ...
            >
              ...
              {(() => {
                ...
                return sensitivities.map((s) => (
                  <SensitivityBar key={s.name} name={s.name} value={s.sensitivity} maxValue={maxSens} color="#C8D8E8" />
                ));
              })()}
            </div>
          ) : (
          ...
          >
            ...
            >
              Positive values (blue) mean velocity increases when the parameter increases. Negative values (red) mean
              velocity decreases. For competitive inhibition, increasing Ki reduces inhibition and increases velocity.
              The larger the absolute value, the more sensitive the system is to that parameter.
            </p>
          </div>
          ...
          {sensitivities.length > 0 && (
            ...
            >
              ...
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: `repeat(${Math.min(sensitivities.length, 3)}, 1fr)`,
                  gap: "6px",
                }}
              >
                {sensitivities.map((s) => (
      ...
      >
        Numerical integration via 4th-order Runge-Kutta · LM parameter estimation · Based on user-provided experimental
        parameters
      </p>
