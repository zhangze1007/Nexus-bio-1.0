...
import { AlertCircle, ChevronDown, ChevronUp, ExternalLink, Loader2, Search, Send } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { useWorkbenchStore } from "../store/workbenchStore";
import { THEME } from "../theme";
import type { COREWorkItem, EuropePMCResultItem, OpenAlexWork, SemanticScholarPaper } from "../types/literature";
...
function decodeOpenAlexAbstract(index: Record<string, number[]> | null | undefined) {
  ...
  const positionedTerms = Object.entries(index)
    .flatMap(([term, positions]) =>
      Array.isArray(positions) ? positions.map((position) => [position, term] as const) : [],
    )
...
function mergeUniqueArticles(existing: Article[], incoming: Article[]) {
  const seen = new Set(existing.map((article) => `${article.doi || article.id || article.title}`.toLowerCase()));
...
async function fetchPubMed(query: string): Promise<Article[]> {
  const searchData = await fetchJsonOrThrow<{ esearchresult?: { idlist?: string[] } }>(
    `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(query)}+AND+(synthetic+biology+OR+metabolic+engineering+OR+fermentation)&retmax=${SOURCE_RESULT_LIMIT}&sort=relevance&retmode=json`,
  );
  const ids: string[] = searchData.esearchresult?.idlist || [];
  if (!ids.length) return [];

  const summaryData = await fetchJsonOrThrow<{ result?: { uids?: string[]; [uid: string]: unknown } }>(
    `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=${ids.join(",")}&retmode=json`,
  );

  return Promise.all(
    (summaryData.result?.uids || []).map(async (uid: string) => {
      const item = summaryData.result?.[uid] as
        | {
            title?: string;
...
async function fetchEuropePMC(query: string): Promise<Article[]> {
  const data = await fetchJsonOrThrow<{ resultList?: { result: EuropePMCResultItem[] } }>(
    `https://www.ebi.ac.uk/europepmc/webservices/rest/search?query=${encodeURIComponent(query)}&format=json&pageSize=${SOURCE_RESULT_LIMIT}`,
  );
...
async function fetchSemanticScholar(query: string): Promise<Article[]> {
  const data = await fetchJsonOrThrow<{ data: SemanticScholarPaper[] }>(
    `https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(query)}&fields=title,abstract,authors,year,journal,externalIds,citationCount,isOpenAccess,venue&limit=${SOURCE_RESULT_LIMIT}`,
  );
...
async function fetchOpenAlex(query: string): Promise<Article[]> {
  const data = await fetchJsonOrThrow<{ results: OpenAlexWork[] }>(
    `https://api.openalex.org/works?search=${encodeURIComponent(query)}&per-page=${SOURCE_RESULT_LIMIT}&mailto=nexusbio@research.com`,
  );
...
async function fetchBioRxiv(query: string): Promise<Article[]> {
  const data = await fetchJsonOrThrow<{ resultList?: { result: EuropePMCResultItem[] } }>(
    `https://www.ebi.ac.uk/europepmc/webservices/rest/search?query=${encodeURIComponent(query)}+AND+SRC:PPR&format=json&pageSize=${SOURCE_RESULT_LIMIT}&sort=FIRST_PDATE:desc`,
  );
  return (data.resultList?.result || []).map((item: EuropePMCResultItem) => ({
    id: `biorxiv-${item.id}`,
    title: item.title || "",
...
async function fetchCORE(query: string): Promise<Article[]> {
  const data = await fetchJsonOrThrow<{ results: COREWorkItem[] }>(
    `https://api.core.ac.uk/v3/search/works?q=${encodeURIComponent(query)}&limit=${SOURCE_RESULT_LIMIT}`,
  );
...
export default function SemanticSearch({ onAnalyzePaper, initialQuery }: SemanticSearchProps) {
  ...
  const sourceState: Record<string, SourceStatus> =
    searchQuery.data?.sourceStates ?? (submittedQuery ? LOADING_SOURCE_STATES : IDLE_SOURCE_STATES);
  ...
  const evidenceIdByKey = useMemo(() => {
    return new Map(evidenceItems.map((item) => [`${item.doi || item.url || item.title}`.toLowerCase(), item.id]));
  }, [evidenceItems]);
  ...
  const openAccessCount = useMemo(() => results.filter((article) => article.openAccess).length, [results]);

  const preprintCount = useMemo(() => results.filter((article) => article.isPreprint).length, [results]);
  ...
  const loadShowcaseAbstract = useCallback(
  ...
  const persistArticleEvidence = useCallback(
    (article: Article, options?: { select?: boolean }) => {
    ...
    >
      <div className="max-w-5xl mx-auto">
        <div style={{ marginBottom: "28px" }}>
          <p
            style={{
              color: RESEARCH_PALETTE.textFaint,
              fontSize: "11px",
              fontFamily: MONO,
          ...
          >
            Search across PubMed, Europe PMC, Semantic Scholar, OpenAlex, bioRxiv, and CORE from one literature desk.
            Nexus-Bio prioritizes open-access and publicly reachable research where available, surfaces live metadata
            from connected sources, and flags source gaps honestly when a service is limited or unavailable.
          </p>
        </div>
        ...
        >
          ...
          >
            Connected sources include biomedical indexes, literature aggregators, preprints, and repository-fed
            open-access records. Five curated synthetic biology classics remain separate from the live query stream so
            background reading never gets mixed into current search results.
          </p>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {SOURCE_DEFINITIONS.map((source) => (
              ...
              >
                ...
                <span style={{ color: RESEARCH_PALETTE.textFaint }}>{SOURCE_DESCRIPTORS[source.key]?.kind}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <form
          ...
          style={{
            ...
            marginBottom: "26px",
            borderRadius: "20px",
            border: isSearchFocused ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(255,255,255,0.09)",
            background: isSearchFocused ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.04)",
        ...
        >
          ...
          >
            <Search
              size={16}
              style={{
                color: isSearchFocused ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.45)",
          ...
          >
            ...
            >
              <div style={{ display: "grid", gap: "4px" }}>
                ...
                >
                  Save the papers that define your target project, then send the selected bundle to Analyze as a
        ...
        {showShowcase && (
          <div style={{ marginBottom: "28px" }}>
            ...
            >
              These five classic papers are curated reference points for synthetic biology and remain separate from live
              query results.
            </p>
            <div style={{ display: "grid", gap: "10px" }}>
              {SHOWCASE_PAPERS.map((paper) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div
                        style={{ marginTop: "14px", paddingTop: "14px", borderTop: "1px solid rgba(255,255,255,0.08)" }}
                      >
                        <div style={{ display: "grid", gap: "12px", marginBottom: "12px" }}>
                          <div style={{ display: "grid", gap: "6px" }}>
                            ...
                            <p
                              style={{ color: "rgba(255,255,255,0.5)", fontSize: "13px", lineHeight: 1.75, margin: 0 }}
                            >
                          ...
                          <div style={{ display: "grid", gap: "6px" }}>
                            ...
                            {showcaseAbstractState[paper.id] === "loading" ? (
                              ...
                              >
                                {highlightKeywords(
                                  showcaseAbstracts[paper.id],
                                  extractKeywords(paper.title, showcaseAbstracts[paper.id]),
                                )}
                              </p>
                            ) : (
                              <p
              ...
              >
                {[
                  { label: "Connected sources", value: `${SOURCE_DEFINITIONS.length}`, detail: "Live public services" },
                  {
                    ...
                    value: `${successfulSources}/${SOURCE_DEFINITIONS.length}`,
                    detail: "Returned this query",
                  },
                  { label: "Unique papers", value: `${results.length}`, detail: "Deduplicated stream" },
                  {
                    label: "Accessible flags",
                    value: `${openAccessCount} OA · ${preprintCount} preprint`,
                    detail: "As flagged by sources",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
              ...
              {failingSources.length > 0 && (
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Some connected sources were limited for this query:{" "}
                      {failingSources.map(({ source, issue }) => `${source} (${issue})`).join(", ")}. The result stream
                      below reflects the remaining live responses.
                    </p>
                  </div>
                </div>
              )}
            ...
            {isSearching && results.length === 0 ? (
              <div style={{ minHeight: "280px" }}>
                <EmptyState
                  ...
                  message="Querying PubMed, Europe PMC, Semantic Scholar, OpenAlex, bioRxiv, and CORE in parallel."
                />
              </div>
              ...
              <div style={{ minHeight: "280px" }}>
                <EmptyState
                  ...
                  message="Try a broader pathway, host organism, or engineering term."
                />
              </div>
            ) : (
              <>
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                                ...
                                <button
                                  ...
                                  style={{
                                    ...
                                    background: "rgba(255,255,255,0.03)",
                                    color:
                                      saveButtonLabel(article) === "Selected" ? "#ffffff" : "rgba(255,255,255,0.6)",
                                    display: "inline-flex",
                                    alignItems: "center",
                                    fontFamily: SANS,
                                    fontSize: "11px",
        ...
        {!hasSearched && !showShowcase && (
          <div style={{ marginTop: "24px" }}>
            <EmptyState
              title="Start with a paper, pathway, or host organism"
              message="Use the search bar to populate the literature workbench."
            />
          </div>
        )}
      </div>
