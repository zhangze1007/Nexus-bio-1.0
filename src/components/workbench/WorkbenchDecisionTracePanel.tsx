...
import { getAuthoritySummary, getAuthorityTier, getFreshnessMap, getToolFreshness } from "./workbenchTrust";
...
export default function WorkbenchDecisionTracePanel({
  ...
  const recommendationFreshness = useMemo(
    () =>
      getFreshnessMap(
        runArtifacts,
        recommendations.map((item) => item.toolId),
  ...
  const rationale = useMemo(() => {
    ...
    if (dbtlPayload?.feedbackSource === "committed") {
      lines.push(
        `Committed DBTL feedback is active: ${dbtlPayload.result.passRate.toFixed(0)}% pass rate at ${dbtlPayload.result.latestPhase}`,
      );
    }
...
function WorkflowStateCard({
  ...
  decision: {
    ...
    missingEvidence: { minRequired: number; have: number };
  };
}) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        ...
        {decision.nextNodeIsContractOnly && <span style={typography.caption}>next: contract-only</span>}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
        ...
        {decision.validity && <span style={typography.caption}>validity · {decision.validity}</span>}
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
        ...
        {toolId && <span style={typography.caption}>{freshness.status}</span>}
      </div>
      {rationale.length ? (
        rationale.map((line) => (
          <div key={line} style={typography.body}>
            {line}
          </div>
        ))
      ) : (
...
function DBTLLedgerCard({
  ...
  runs: {
    ...
    evidenceSnapshot?: { count?: number; status?: string; missingEvidence?: { minRequired?: number } } | null;
  }[];
  decision: {
    missingEvidence: { have: number };
    confidence: number | null;
    uncertainty: number | null;
    validity: string | null;
    humanGateRequired: boolean;
  ...
  return (
    <motion.div variants={cardVariants} style={glassPanel}>
      ...
      <div style={{ display: "grid", gap: "10px" }}>
        {runs.length ? (
          runs.map((run) => <LedgerEntry key={run.id} run={run} decision={decision} />)
        ) : (
          <div style={typography.body}>Run PATHD to create the first auditable DBTL decision artifact.</div>
        )}
      </div>
    </motion.div>
  );
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
        ...
        <span style={typography.caption}>
          ...
          {run.confidence !== undefined && run.confidence !== null
            ...
            : decision.latestRunToolId === run.toolId && decision.confidence !== null
              ? decision.confidence.toFixed(2)
              : "n/a"}
        </span>
        <span style={typography.caption}>
          uncertainty ·{" "}
          {run.uncertainty !== undefined && run.uncertainty !== null
            ? run.uncertainty.toFixed(2)
            : decision.latestRunToolId === run.toolId && decision.uncertainty !== null
              ? decision.uncertainty.toFixed(2)
              : "unknown"}
        </span>
        <span style={typography.caption}>
        ...
        <span style={typography.caption}>
          human gate ·{" "}
          {(run.humanGateRequired ?? (decision.latestRunToolId === run.toolId ? decision.humanGateRequired : false))
            ? "yes"
            : "no"}
        </span>
        ...
        <span style={typography.caption}>
          provenance · {provenanceDiagnostics.chainLength > 0 ? "present" : "missing"}
        </span>
        <span style={typography.caption}>chain · {provenanceDiagnostics.chainLength}</span>
        {provenanceDiagnostics.hasMissingUpstream && (
          <span style={typography.caption}>
            missing upstream · {provenanceDiagnostics.missingUpstreamProvenanceIds.length}
          </span>
        )}
      </div>
    </motion.div>
    ...
    <motion.div variants={cardVariants} whileHover="hover">
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          ...
          <span style={typography.body}>{freshness?.summary ?? "No execution integrity signal yet."}</span>
        </div>
      </Link>
    </motion.div>
  );
