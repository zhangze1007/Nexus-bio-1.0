...
import { buildWorkflowDashboardItems, type WorkflowExperienceStatus, workflowStatusLabel } from "./workflowExperience";
...
export default function WorkbenchStatusBar({ moduleId }: WorkbenchStatusBarProps) {
  ...
  const syncLabel = useMemo(() => {
    ...
    if (syncStatus === "synced")
      return lastServerSyncAt
        ? `Synced ${new Date(lastServerSyncAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
        : "Synced";
  ...
  const workflowProgress = useMemo(() => {
    const complete = workflowDashboardItems.filter(
      (item) => item.id !== "target" && item.id !== "nexai" && item.status === "complete",
    ).length;
    return `${complete}/6`;
  ...
  const freshnessLabel = moduleId
    ? freshness.status === "fresh"
      ...
      : freshness.status === "stale"
        ? `Stale: ${freshness.blockingToolIds.map((id) => id.toUpperCase()).join(", ")}`
                    ...
                    >
                        ...
                        >
                          <Workflow
                            ...
                            color={getWorkflowStatusColor(
                              workflowControl.status === "gated" ? "humanGate" : workflowControl.status,
                            )}
                          />
                        ...
                        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                          {workflowDashboardItems.map((item) => {
                            ...
                            return item.href ? (
                              ...
                              <div key={item.id}>{itemEl}</div>
                            );
                          })}
                        </div>
                        <div style={{ fontFamily: THEME.MONO, fontSize: "10px", color: LABEL, marginTop: "6px" }}>
                          {workflowControl.currentToolId && (
                            <span>current: {workflowControl.currentToolId.toUpperCase()} · </span>
                          )}
                          {workflowControl.confidence !== null && (
                            <span>conf {workflowControl.confidence.toFixed(2)} · </span>
                          )}
                          <span>
                            uncert {workflowControl.uncertainty === null ? "?" : workflowControl.uncertainty.toFixed(2)}
                          </span>
                          {workflowControl.humanGateRequired && (
                            <span style={{ color: THEME.APRICOT }}> · human gate</span>
                          )}
                        </div>
                      </DrawerSection>
                      ...
                      {moduleId && (
                        <DrawerSection
                          ...
                          accent={freshness.status === "stale" ? THEME.CORAL : THEME.MINT}
                        >
                  ...
                  {drawerTab === "evidence" && (
                    ...
                    >
                          ...
                          selectedEvidence.map((item) => (
                            <div key={item.id} style={{ ...glassPanel, padding: "10px 12px" }}>
                              ...
                              <div style={typography.body}>
                                ...
                                {item.abstract.length > 160 ? "…" : ""}
                              </div>
                            </div>
                          ))
                        ) : (
                          <div style={typography.body}>
                            No evidence selected. Save papers in Research to build a bundle.
                          </div>
                        )}
                      </DrawerSection>
                      ...
                      <DrawerSection title="Analyze Artifact" accent={THEME.LILAC}>
                        {analyzeArtifact ? (
                          <div style={{ ...glassPanel, padding: "10px 12px" }}>
                            ...
                            <div style={typography.caption}>
                              {analyzeArtifact.nodes.length} nodes · {analyzeArtifact.edges.length} edges ·{" "}
                              {analyzeArtifact.bottleneckAssumptions.length} bottlenecks
                            </div>
                          </div>
                        ) : (
                      ...
                      <WorkbenchDecisionTracePanel
                        toolId={moduleId}
                        title={moduleId ? "Decision Trace" : "Workbench Decision Trace"}
                        limit={4}
                      />
                    </motion.div>
                  )}
                  ...
                  {drawerTab === "history" && (
                    ...
                    >
                      <WorkbenchRunCompare
                        ...
                        title={moduleId ? "Tool Compare" : "Stage Compare"}
                      />
                      <WorkbenchProjectTimeline
                        title={moduleId ? "Project Timeline" : "Workbench Timeline"}
                        limit={5}
                      />
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...glassPanel,
          ...(accent ? accentLeftBorder(accent, 2) : {}),
          padding: "10px 12px",
          display: "grid",
          gap: "6px",
        }}
