...
export function getWorkbenchActorId() {
  ...
  try {
    ...
    const generated =
      typeof window.crypto?.randomUUID === "function" ? `actor-${window.crypto.randomUUID()}` : createId("actor");
...
export function composeEvidenceText(items: WorkbenchEvidenceItem[]) {
  return items
    .map((item) => {
      const meta = [item.source ?? item.journal, item.year, item.doi ? `DOI: ${item.doi}` : null]
        .filter(Boolean)
        .join(" · ");
      return [
        `Title: ${item.title}`,
