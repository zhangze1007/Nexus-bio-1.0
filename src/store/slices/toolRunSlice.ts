...
import { getNextToolIds, getStageForTool } from "../../components/tools/shared/workbenchConfig";
  ...
  setToolPayload: (toolId, payload) => {
    set((state) => {
      ...
      const executionStable =
        latestArtifactForTool?.execution.dependencySignature === nextExecution.dependencySignature;
      ...
      const blocksCanonicalPayload = contract?.contractScope === "workflow" && runArtifact.status !== "ok";
      ...
      const toolPayloads =
        blocksCanonicalPayload || trustBlocked
          ? state.toolPayloads
          : {
              ...state.toolPayloads,
      ...
      const allowedDownstreamToolIds = downstreamToolIds.filter(
        (nextToolId) => canPassToDownstream(admittedPayload, nextToolId).allowed,
      );
      const blockedDownstreamToolIds = downstreamToolIds.filter(
        (nextToolId) => !canPassToDownstream(admittedPayload, nextToolId).allowed,
      );
      const recommendationToolIds = blocksCanonicalPayload
        ? (runArtifact.blockingUpstreamToolIds ??
          (workflowControl.nextRecommendedNode ? [workflowControl.nextRecommendedNode] : []))
