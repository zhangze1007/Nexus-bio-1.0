...
function pushIssue(
  issues: LearnedDeltaValidationIssue[],
  issue: LearnedDeltaValidationIssue,
): void {
  issues.push(issue);
}
...
export function validateLearnedDeltaPack(
  value: unknown,
): LearnedDeltaValidationResult {
  ...
  const hasMissingDbtlSource = issues.some((issue) =>
    issue.field === 'sourceDbtlRunId' && issue.severity === 'error'
  );
...
export function isLearnedDeltaPack(
  value: unknown,
): value is LearnedDeltaPack {
