...
import type { BiosafetyInput, BiosafetyOutput, SequenceMatch, ContainmentStrategy } from './types';
...
export function assessBiosafety(input: BiosafetyInput): BiosafetyOutput {
  ...
  const selectAgentScore = selectAgentMatches.length > 0
    ? Math.max(...selectAgentMatches.map(m => m.score))
    : 0;
  const virulenceScore = virulenceMatches.length > 0
    ? Math.max(...virulenceMatches.map(m => m.score))
    : 0;
  const resistanceScore = resistanceMatches.length > 0
    ? Math.max(...resistanceMatches.map(m => m.score))
    : 0;
