 ...
 */
async function predictMultiChainComplex(
  chains: ProteinChain[],
): Promise<{
  pdb: string;
  plddt: number[];
...
function findInterfaceResidues(
  ...
  const chainAtoms: Map<string, Array<{ index: number; residue: string; atom: string; x: number; y: number; z: number }>> = new Map();
...
export async function predictStructure(input: StructureInput): Promise<StructureResult> {
  ...
  if (chainResults.length >= 2 && chainResults.every(r => r.pdb)) {
    ...
    interfaceResidues = findInterfaceResidues(combinedPdb, chainResults.map(r => r.chainId));
  }

  const avgPLDDT = chainResults.length > 0
    ? chainResults.reduce((s, r) => s + r.avgPLDDT, 0) / chainResults.length
    : 0;
