...
export function Input({
  ...
  const inputContainerStyle: React.CSSProperties = useMemo(
    () => ({
      ...
      border: `1px solid ${error ? colors.state.error : focused ? colors.border.accent : colors.border.default}`,
      borderRadius: borderRadius.md,
      padding: `${spacing.md} ${spacing.base}`,
      minHeight: "44px",
      boxShadow: focused ? (error ? shadows.glowError : shadows.glowPrimary) : shadows.inner,
  ...
  const iconStyle: React.CSSProperties = useMemo(
    () => ({
      ...
      width: "18px",
      height: "18px",
      color: error ? colors.state.error : focused ? colors.accent.primary : colors.text.tertiary,
  ...
  const labelStyle: React.CSSProperties = useMemo(
    () => ({
      ...
      fontWeight: isFloating ? typography.fontWeight.medium : typography.fontWeight.regular,
      color: error ? colors.state.error : focused ? colors.accent.primary : colors.text.tertiary,
      pointerEvents: "none",
      transition: [
