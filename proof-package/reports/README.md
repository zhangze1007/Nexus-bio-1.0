# Reports

This directory contains copied local-dev reports:

- `trust-metrics-latest.json`: Step 15 trust metrics report.
- `public-benchmark-report.json`: Step 17 public baseline comparison metadata.
- `public-benchmark-summary.csv` and `.json`: Step 17 per-mode summary.
- `public-benchmark-raw-results.csv` and `.json`: Step 17 per-case, per-mode rows.

Refresh these files from repo-root generated reports with:

```bash
npm run proof:replay
```

These reports are local development benchmark artifacts. They are not external validation, scientific model validation, wet-lab evidence, release certification, or user study results.
